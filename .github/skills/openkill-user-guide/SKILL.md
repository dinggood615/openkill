---
name: openkill-user-guide
description: 'OpenKill 用户功能指南。用于回答用户关于 OpenKill 插件如何启用/关闭各项功能的问题，包括：运行模式切换、代理开关、DNS 设置、流量控制、访问控制黑白名单、IPv6 开关、规则/GEO 更新、自动重启、仪表盘设置、订阅管理、覆写设置等。每个选项均标注了对应的 UCI 配置项、修改的 Mihomo YAML 配置段、以及触发的脚本。Use when user asks how to enable, disable, configure, or troubleshoot any OpenKill feature on OpenWrt.'
instructions: |
  You are an OpenKill expert assistant. OpenKill is a LuCI plugin for OpenWrt that manages the Mihomo (Clash Meta) proxy kernel.

  When answering user questions about OpenKill:
  1. When users report any issue (cannot access internet, DNS failure, startup failure, etc.), FIRST ask them to generate a debug log — either via LuCI (运行日志 → 生成日志) or SSH (`/usr/share/openkill/openkill_debug.sh`). The debug log contains dependency checks, config, firewall rules, and system info in one step.
  ...
  3. Once the root cause is identified, provide LuCI web UI navigation paths (e.g. 服务 → OpenKill → 插件设置 → 流量控制) to fix the configuration.
  ...
  6. Never guess — if information is not covered in this document, actively query: Mihomo Wiki (https://wiki.metacubex.one/config/), Meta-Docs (https://github.com/MetaCubeX/Meta-Docs), OpenKill source code (https://github.com/vernesong/OpenClash/tree/dev), Mihomo core source code (https://github.com/MetaCubeX/mihomo/tree/Alpha), Smart core source code (https://github.com/vernesong/mihomo/tree/Alpha). For bugs/errors, also search: OpenKill Issues (https://github.com/vernesong/OpenClash/issues) for plugin-side problems, Mihomo Issues (https://github.com/MetaCubeX/mihomo/issues) for core-side problems.
  7. Cite sources when information comes from external queries.

type: knowledge-base
tags: [openkill, openwrt, mihomo, clash, proxy, networking]
argument-hint: '询问 OpenKill 功能如何开启/关闭'
user-invocable: true
...
> **本文档是 AI 的知识库，而非给用户看的说明书。任何 AI 模型（Copilot / Claude / ChatGPT / Gemini / DeepSeek 等）在回答 OpenKill 相关问题时均应遵循以下原则。**
>
> **排查优先级（从快到慢，逐层递进）**：
...
| 原则 | 说明 |
|------|------|
| **主动查证，不猜测** | 遇到本文档未覆盖的 Mihomo 配置字段或实现细节时，**禁止编造**。必须使用工具主动查询外部资源（Mihomo Wiki / Meta-Docs / Mihomo 核心源码 / OpenKill 源码 / Smart 核心源码），整理后告知用户。详见文档末尾「超出本文档范围的查询」节。 |
| **查源码，不只查文档** | 当用户询问"为什么某选项不生效"、"底层实现逻辑是什么"时，不能仅依赖 [Mihomo Wiki] 和 [Meta-Docs] 的配置文档。必须进一步查阅 [Mihomo 核心源码](https://github.com/MetaCubeX/mihomo/tree/Alpha)、[OpenKill 源码](https://github.com/vernesong/OpenClash/tree/dev) 和 [Smart 核心源码](https://github.com/vernesong/mihomo/tree/Alpha) 中的对应脚本/函数，理解实际执行逻辑。 |
| **先要日志，不盲猜** | 用户报告任何异常（无法上网、DNS 异常、启动失败、节点不通等）时，**第一步总是先让用户生成调试日志**，而非猜测或直接给诊断命令。调试日志一键包含依赖检查、运行状态、防火墙规则、系统信息等 20+ 章节，比逐条执行诊断命令高效得多。生成方式：① **LuCI 页面**：「运行日志」→「生成日志」按钮；② **SSH 命令**：`/usr/share/openkill/openkill_debug.sh`（输出 `/tmp/openkill_debug.log`）。拿到日志后对照「日志与错误信息速查」和第七部分决策树进行诊断。 |
| **日志不足再给命令** | 仅当调试日志不足以定位问题时，才按第七部分的诊断决策树给用户精确的 CLI 诊断命令。优先使用 🟢 安全查询命令，对 🟡/🔴 命令附带风险说明。用户执行后粘贴输出，AI 分析结果决定下一步。 |
| **配置给路径，修复给步骤** | 功能配置（如何开启/关闭/设置选项）和问题修复 → 给出 LuCI Web 界面操作路径（如「服务 → OpenKill → 插件设置 → 流量控制」），而非命令行。仅在用户明确要求 CLI 操作或 LuCI 不可用时才提供终端命令。 |
...
| **查 Issues，不闭门造车** | 当用户遇到的功能问题在本文档中未覆盖，或报错信息在错误速查表中无匹配项时，**必须主动搜索 Issues** 查找是否存在相同或相似的问题：① 插件配置/订阅/防火墙/UI 相关问题 → 搜索 [OpenKill Issues](https://github.com/vernesong/OpenClash/issues)；② 内核级问题（代理协议/TUN/DNS 解析/规则引擎等 Mihomo 核心行为） → 搜索 [Mihomo Issues](https://github.com/MetaCubeX/mihomo/issues)。优先参考：**作者/维护者的回复**（OpenKill 标有 Owner 标签的 vernesong；Mihomo 标有 Contributor/Collaborator 标签的回复）——代表官方立场或已知 bug；**高赞反应（👍）的社区回复**——代表经过验证的有效方案；**同类问题中的诊断命令**（如 `nft list set`、`dig`、`uci show` 等）——可直接复用于用户的问题排查。搜索时使用用户报错中的关键错误信息或功能描述作为关键词。 |
...
| OpenKill Issues | `https://github.com/vernesong/OpenClash/issues` | 搜索插件侧已知问题、社区方案、作者回复 |
...
| OpenKill 源码 | `https://github.com/vernesong/OpenClash/tree/dev` | 插件实现逻辑（Shell/Ruby/Lua 脚本） |
...
> **AI 行为指引**: 当用户报告启动失败、功能异常时，AI 应**先让用户生成调试日志**（LuCI「运行日志」→「生成日志」或 SSH `openkill_debug.sh`），然后对照日志中的 `#===== 依赖检查 =====#` 段检查依赖完整性。对于缺失的依赖，指导用户在 LuCI 的「系统 → 软件包」中搜索安装。
>
> **固件提醒**: 推荐使用 ImmortalWrt 或 OpenWrt 官方固件（需自行将 `dnsmasq` 替换为 `dnsmasq-full`）。不推荐使用第三方魔改/高大全固件、以及已停止维护的旧版固件。旁路由组网存在固有的网络层面缺陷，强烈建议采用主路由架构部署 OpenKill。

### 一、包依赖总览（来自 Makefile DEPENDS 和 init.d 运行时检查）

OpenKill 依赖以下软件包，由 `opkg`/`apk` 在安装时自动拉取。若手动卸载了其中某个包，会导致对应功能异常。
...
| 要求 | 说明 |
|------|------|
| **必须使用 `dnsmasq-full`** | OpenWrt 自带的 `dnsmasq` 精简版缺少 ipset/nftset 支持，OpenKill 的 DNS 劫持和 chnroute 旁路依赖此功能 |
...
`init.d/openkill` 的 `check_mod()` 函数以四级回退方式检查和加载内核模块：
...
### 五、更新后自动修复依赖（`openkill_update.sh`）
...
│       │  UCI 读写    │            │                              │
│       ▼             ▼            ▼                              │
│  /etc/config/openkill  — UCI 配置文件 (所有选项持久化在此)       │
│       │                                                         │
│       ▼ Shell Scripts ( /usr/share/openkill/ )                 │
│  ┌──────────────────────────────────────────────────────┐       │
│  │ openkill.sh        → 订阅下载/更新/节点过滤           │       │
│  │ openkill_core.sh   → 核心二进制更新                  │       │
│  │ openkill_update.sh → 插件 IPK 更新                   │       │
│  │ openkill_geo.sh    → GEO 数据库下载 (ipdb/dat/geosite/asn)  │
│  │ openkill_chnroute.sh → 大陆 IP 路由表更新             │       │
│  │ yml_change.sh       → Ruby 修改 YAML (端口/模式/DNS/TUN/认证) │
│  │ yml_rules_change.sh → Ruby 修改 YAML (规则/Provider/URL-Test) │
│  │ openkill_debug.sh  → 诊断日志收集                    │       │
│  │ openkill_watchdog.sh → 核心存活 + 流媒体解锁守护       │       │
│  └──────────────────────────────────────────────────────┘       │
│       │                                                         │
│       ▼ 生成 / 覆写                                              │
│  /etc/openkill/config/*.yaml  — 原始订阅配置                    │
│  /etc/openkill/*.yaml          — 经脚本处理后的运行配置          │
│  /etc/openkill/overwrite/     — 覆写模块文件                    │
│  /etc/openkill/custom/        — 用户自定义规则/DNS/防火墙脚本    │
│       │                                                         │
│       ▼                                                         │
│  /etc/openkill/clash  — symlink → /etc/openkill/core/clash_meta│
│  /etc/openkill/        — GEO 数据: Country.mmdb, GeoSite.dat 等 │
└─────────────────────────────────────────────────────────────────┘
...
| 路径 | 作用 |
|------|------|
| `/etc/config/openkill` | UCI 配置文件，所有 LuCI 选项持久化在此 |
| `/etc/openkill/` | OpenKill 工作目录（核心、GEO 数据、Chnroute 列表） |
| `/etc/openkill/config/` | 原始订阅配置存放目录（`.yaml` 文件，经 `yml_change.sh` 处理后生成 `/etc/openkill/<name>.yaml` 运行配置） |
| `/etc/openkill/overwrite/` | 覆写模块文件（INI 格式，定义自定义 YAML 覆盖） |
| `/etc/openkill/custom/` | 用户自定义文件（规则列表、DNS 策略、Hosts、防火墙脚本、Sniffer 配置等） |
| `/etc/openkill/core/` | 核心二进制存放目录（多版本共存，/etc/openkill/clash 是到 core/clash_meta 的 symlink） |
| `/etc/openkill/dashboard/` | Dashboard 静态文件（yacd/metacubexd/zashboard） |
| `/etc/openkill/Model.bin` | LightGBM 智能策略模型文件（注意：不是目录，是单个 .bin 文件） |
| `/usr/share/openkill/` | 插件脚本目录（Shell/Ruby/Lua 脚本） |
| `/tmp/openkill.log` | 运行日志 |
| `/tmp/openkill_start.log` | 启动日志 |
| `/tmp/etc/openkill/` | 小闪存模式下的工作目录（重启后清空） |
| `/var/etc/openkill.include` | 防火墙规则加载文件（由 firewall UCI 自动 include） |

- **UCI 配置根**: `openkill` (所有选项均在 `uci show openkill` 可见)
...
- **核心启动脚本**: `/etc/init.d/openkill {start|stop|restart|reload|enable|disable}`
- **自定义文件目录**: `/etc/openkill/custom/` — 存放用户自定义规则/DNS/防火墙脚本
...
```
/etc/init.d/openkill start_service()
│
├─ 第1步: 读取配置
...
│
├─ 第2步: 环境准备
│   ├─ do_run_file()        → 检查/下载核心二进制 (/etc/openkill/core/clash_meta)
│   ├─ 创建 symlink         → ln -s /etc/openkill/core/clash_meta /etc/openkill/clash
│   └─ 小闪存模式处理       → 将文件移到 /tmp/etc/openkill
│
...
│
├─ 第4步: 启动核心
│   └─ procd 启动 clash -d /etc/openkill -f <config.yaml>
...
└─ 第6步: 定时任务 + 守护进程
    ├─ add_cron()            → 注册 cron 任务
    │   ├─ openkill.sh      → 定时更新订阅
    │   ├─ openkill_geo.sh  → 定时更新 GEO 数据
    │   ├─ openkill_chnroute.sh → 定时更新大陆路由
    │   └─ /etc/init.d/openkill restart → 定时自动重启
    └─ start_watchdog()      → 启动守护进程
        ├─ openkill_watchdog.sh        → 核心存活监控
        └─ openkill_streaming_unlock.lua → 流媒体解锁守护
```
...
| 修改防火墙规则 | `/etc/init.d/openkill reload` | 即时 |
...
> OpenKill 同时支持 **fw3 (iptables/ipset)** 和 **fw4 (nftables)** 两种防火墙后端，通过 `command -v fw4` 自动检测：
...
> 如涉及底层实现细节，查阅 [OpenKill 源码](https://github.com/vernesong/OpenClash/tree/dev) 中
> `/etc/init.d/openkill` 的 `set_firewall()` 函数。
...
> `fw4_has_dns_hijack_rule()` 函数在插入前检查 dstnat 链是否已有 OpenKill DNS Hijack 规则，避免重复。
...
nft insert rule inet fw4 dstnat position 0 \
  ...
  counter redirect to <dnsmasq_port> comment "OpenKill DNS Hijack"

# 白名单模式 (lan_ac_mode=1): 仅白名单 IP/MAC 走劫持
nft insert rule inet fw4 dstnat position 0 \
  meta nfproto {ipv4} meta l4proto {tcp,udp} th dport 53 \
  ip saddr @lan_ac_white_ips counter redirect to <dnsmasq_port> comment "OpenKill DNS Hijack"
nft insert rule inet fw4 dstnat position 0 \
  meta nfproto {ipv4} meta l4proto {tcp,udp} th dport 53 \
  ether saddr @lan_ac_white_macs counter redirect to <dnsmasq_port> comment "OpenKill DNS Hijack"
...
nft insert rule inet fw4 nat_output position 0 \
  ...
  ip daddr {127.0.0.1} counter redirect to <dnsmasq_port> comment "OpenKill DNS Hijack"
```
...
# 黑名单模式: 排除黑名单设备
nft add rule inet fw4 openkill_dns_redirect \
  meta nfproto {ipv4} meta l4proto {tcp,udp} th dport 53 \
  ip saddr != @lan_ac_black_ips ether saddr != @lan_ac_black_macs \
  counter redirect to <dns_port> comment "OpenKill DNS Hijack"

# 白名单模式: 仅白名单设备走劫持
nft add rule inet fw4 openkill_dns_redirect \
  meta nfproto {ipv4} meta l4proto {tcp,udp} th dport 53 \
  ip saddr @lan_ac_white_ips counter redirect to <dns_port> comment "OpenKill DNS Hijack"
nft add rule inet fw4 openkill_dns_redirect \
  meta nfproto {ipv4} meta l4proto {tcp,udp} th dport 53 \
  ether saddr @lan_ac_white_macs counter redirect to <dns_port> comment "OpenKill DNS Hijack"

nft insert rule inet fw4 dstnat position 0 \
  meta nfproto {ipv4} meta l4proto {tcp,udp} th dport 53 counter jump openkill_dns_redirect

# === OUTPUT (仅 router_self_proxy=1): 路由器自身 DNS 直接到 dns_port ===
nft add chain inet fw4 nat_output { type nat hook output priority -1; }
nft insert rule inet fw4 nat_output position 0 \
  meta nfproto {ipv4} meta l4proto {tcp,udp} th dport 53 \
  ip daddr {127.0.0.1} meta skgid != 65534 counter redirect to <dns_port> comment "OpenKill DNS Hijack"
```
...
| 链名 | 钩子来源 | 协议 | 动作 | 触发条件 |
|------|----------|------|------|----------|
| `openkill` | `dstnat` jump | TCP | REDIRECT → `$proxy_port`(7892) | 始终 |
| `openkill_mangle` | `mangle_prerouting` jump | UDP | TPROXY → `:$tproxy_port`(7895), mark `0x162` | `enable_udp_proxy=1` 或 Fake-IP 模式 |
| `openkill_upnp` | `openkill_mangle` jump | UDP | UPNP 端口排除 (RETURN) | 自动检测 upnpd |
| `openkill_output` | `nat_output` jump | TCP | 路由器自身 TCP REDIRECT | `router_self_proxy=1` 或 Fake-IP 模式 |
| `openkill_mangle_output` | `mangle_output` jump | UDP | 路由器自身 UDP 标记 | `router_self_proxy=1`+`enable_udp_proxy=1` 或 Fake-IP |

**`openkill` 链规则排序 (TCP REDIRECT)** — 与 init.d 实际代码一致:
...
nft add rule inet fw4 openkill ether saddr != @lan_ac_white_macs \
...
nft add rule inet fw4 openkill ip protocol tcp \
...
**`openkill_mangle` 链规则排序 (UDP TPROXY)** — 仅在 `enable_udp_proxy=1` 或 Fake-IP 模式时创建:
...
nft add rule inet fw4 openkill_mangle meta l4proto {udp} \
...
> **注意**: 当 `enable_udp_proxy != 1` 但 `en_mode = fake-ip` 时，仍会创建简化的 `openkill_mangle` 链仅处理 Fake-IP UDP 流量（无 common_ports/china_ip_route/UPNP 检查）。`ip rule add fwmark $PROXY_FWMARK table $PROXY_ROUTE_TABLE` + `ip route add local 0.0.0.0/0 dev lo table $PROXY_ROUTE_TABLE` 在 UDP TPROXY 启用时创建策略路由。
...
| 链名 | 钩子来源 | 协议 | 动作 | 触发条件 |
|------|----------|------|------|----------|
| `openkill_mangle` | `mangle_prerouting` jump | TCP+UDP | 设置 fwmark `0x162` | 始终 |
| `openkill_mangle_output` | `mangle_output` jump | TCP+UDP | 路由器自身 fwmark | `router_self_proxy=1` 或 Fake-IP |
| `openkill_upnp` | `openkill_mangle` jump | UDP | UPNP 端口排除 (RETURN) | 自动检测 |

**`openkill_mangle` 规则排序 (TUN 模式)** — 与 init.d 实际代码一致:

```bash
# 1. 跳过 TUN 接口自身流量 (防止回环)
nft add rule inet fw4 openkill_mangle meta l4proto {tcp,udp} \
...
# 4. LAN 白名单非匹配 RETURN
nft add rule inet fw4 openkill_mangle ether saddr != @lan_ac_white_macs \
  ip saddr != @lan_ac_white_ips counter return
...
nft add rule inet fw4 openkill_mangle \
...
nft add rule inet fw4 openkill_mangle meta nfproto {ipv4} \
  ip protocol icmp icmp type echo-request mark set $PROXY_FWMARK counter accept \
  comment "OpenKill ICMP Mark"
...
nft insert rule inet fw4 forward position 0 oifname utun udp dport 443 \
  ip daddr != @china_ip_route counter reject comment "OpenKill QUIC REJECT"
...
| nftables 链 | 功能 | 触发条件 |
|-------------|------|----------|
| `openkill_v6` | IPv6 TCP REDIRECT | `ipv6_mode=1` 或 `3` |
| `openkill_mangle_v6` | IPv6 TPROXY / TUN fwmark | `enable_v6_udp_proxy=1` 或 `ipv6_mode≠1` |
| `openkill_output_v6` | 路由器自身 IPv6 TCP | `router_self_proxy=1` + (`ipv6_mode=1` 或 `3`) |
| `openkill_mangle_output_v6` | 路由器自身 IPv6 fwmark | `router_self_proxy=1` |
| `openkill_post_v6` | 旁路由 SNAT/MASQUERADE | `bypass_gateway_compatible=1` |
| `openkill_wan6_input` | 仅内网 IPv6 WAN 防护 | `intranet_allowed=1` |

**IPv6 链规则结构** (以 `openkill_mangle_v6` 为例，包含 TProxy/TUN/Mix 所有模式的综合处理):
...
nft add rule inet fw4 openkill_mangle_v6 meta nfproto {ipv6} \
  ip6 nexthdr icmpv6 icmpv6 type echo-request mark set $PROXY_FWMARK counter accept \
  comment "OpenKill ICMPv6 Redirect"
...
nft insert rule inet fw4 input position 0 udp dport 443 \
  ip6 daddr != @china_ip6_route counter reject comment "OpenKill QUIC REJECT"
nft insert rule inet fw4 forward position 0 [oifname utun] udp dport 443 \
  ip6 daddr != @china_ip6_route counter reject comment "OpenKill QUIC REJECT"
```
...
OpenKill 对 ICMP（ping）请求的处理**取决于运行模式**：

**1. 非 TUN 模式（Redir-Host / Fake-IP，`en_mode_tun` 为空）**:

ICMP echo-request 在 `openkill_mangle` 链中被**仅标记 fwmark（0x162）但不重定向**：

```bash
nft add rule inet fw4 openkill_mangle ip protocol icmp \
  icmp type echo-request mark set "$PROXY_FWMARK" counter accept comment "OpenKill ICMP Mark"
```
...
- **路由器自身 ICMP**：当 `router_self_proxy=1` 时，路由器发出的 ping 在 `openkill_mangle_output` 链中同样被标记。

**2. TUN 模式（`en_mode_tun=1`）**:

ICMP echo-request 在 `openkill_mangle` 链中被标记 fwmark，随后通过策略路由进入 TUN 虚拟网卡：
...
# OUTPUT 链 — 阻止路由器发出对 Fake-IP 的 ping（排除 OpenKill 自身进程 skgid=65534）
...
这是因为在非 TUN 模式下，Fake-IP 地址没有对应的 TCP/UDP 重定向路径（TCP 走 REDIRECT、UDP 走 TPROXY，但 ICMP 都不到达内核），发往这些地址的 ping 无意义且会干扰网络诊断。OUTPUT 链排除 `skgid=65534` 是为了避免影响 OpenKill 自身进程的内部通信。
...
```bash
nft add rule inet fw4 openkill_mangle_v6 ip6 nexthdr icmpv6 \
...
|----------|------------|------------|------------|------------|
| **入站** (LAN→路由器) | `openkill` (非TUN) / `openkill_mangle` (TUN) | `openkill_mangle` | `openkill_v6` / `openkill_mangle_v6` | `openkill_mangle_v6` |
| **出站** (路由器自身) | `openkill_output` (非TUN) / `openkill_mangle_output` (TUN) | `openkill_mangle_output` | `openkill_output_v6` / `openkill_mangle_output_v6` | `openkill_mangle_output_v6` |
| **旁路由 SNAT** | `openkill_post` | `openkill_post` | `openkill_post_v6` | `openkill_post_v6` |

**规则格式示例** (fw4 nftables):

```bash
# 入站规则: 跳过代理
nft insert rule inet fw4 openkill position 0 tcp \
  sport 0-65535 meta nfproto {ipv4} ip daddr != {<fakeip_range>} \
  ip saddr {192.168.1.100} counter return comment "my_device_bypass"

# OUTPUT 规则: 含 user 匹配
nft insert rule inet fw4 openkill_output position 0 tcp \
...
|-------------|-----|-----------------|
| `openkill` | `nat` | `inet fw4 openkill` (TCP) |
| `openkill` | `mangle` | `inet fw4 openkill_mangle` (UDP) |
| `openkill_output` | `nat` | `inet fw4 openkill_output` (TCP) |
| `openkill_output` | `mangle` | `inet fw4 openkill_mangle_output` (UDP) |
| `openkill_post` | `nat` | `inet fw4 openkill_post` |
| `openkill_wan_input` | `filter` | `inet fw4 openkill_wan_input` |
| `openkill_dns_redirect` | `nat` | `inet fw4 openkill_dns_redirect` |
| `openkill_upnp` | `mangle` | `inet fw4 openkill_upnp` |
...
iptables -t nat -A openkill -p tcp -d 198.18.0.0/16 -j REDIRECT --to-ports 7892
iptables -t nat -A openkill -p tcp -m set ! --match-set common_ports dst -j RETURN
iptables -t nat -A openkill -p tcp -j REDIRECT --to-ports 7892
...
iptables -t mangle -A openkill -p udp -m set --match-set localnetwork dst -j RETURN
iptables -t mangle -A openkill -p udp -j TPROXY --on-port 7895 --tproxy-mark 0x162
...
| **`router_self_proxy`** (路由本机代理 / Router-Self Proxy) | `1` | 创建 OUTPUT 链 (`openkill_output` + `openkill_mangle_output`)，路由器自身流量被重定向/标记。非 TUN 模式额外对 Fake-IP 模式始终创建 OUTPUT 链（即使用户关闭 router_self_proxy） |
| | `0` | 删除 OUTPUT 链，路由器自身流量走原始路由 |
| **`intranet_allowed`** (仅允许内网 / Only Intranet Allowed) | `1` | IPv4: 创建 `openkill_wan_input` 链，REJECT 来自 WAN 口对全部服务端口的访问。IPv6: 创建 `openkill_wan6_input` 链。服务端口: `$proxy_port`(7892)、`$tproxy_port`(7895)、`$cn_port`(9090)、`$http_port`(7890)、`$socks_port`(7891)、`$mixed_port`(7893)、`$dns_port`(7874) |
| **`bypass_gateway_compatible`** (旁路网关（旁路由）兼容 / Bypass Gateway Compatible) | `1` | IPv4: 创建 `openkill_post` 链 (srcnat jump)，对已标记流量执行 MASQUERADE SNAT。规则: skgid return → mark accept → localnetwork return → ct reply return → fib saddr 非 local masquerade。IPv6: 对应创建 `openkill_post_v6` 链 |
...
| | `2` | 创建 `openkill_dns_redirect` 链，IPv4+IPv6 DNS 流量直接 DNAT 到 `dns_port`(7874)。同样支持 AC 过滤和 OUTPUT 劫持 |
...
| **UPNP 流量排除**（无 UCI 选项，自动检测 `/etc/config/upnpd` 租约文件） | 系统已安装 upnpd | 创建 `openkill_upnp` 链，`upnp_exclude()` 遍历 upnpd 租约文件，按 `saddr + sport + protocol` 三元组为每个映射添加 RETURN 规则。看门狗自动同步变更 |
| **`ipv6_enable`** (IPv6 流量代理 / Proxy IPv6 Traffic) | `1` | 创建完整 IPv6 防火墙链：`openkill_v6`(TCP REDIRECT, ipv6_mode=1/3)、`openkill_mangle_v6`(UDP TPROXY/TUN fwmark)、`openkill_output_v6`/`openkill_mangle_output_v6`(路由自身)、`openkill_post_v6`(旁路由 SNAT)、`openkill_wan6_input`(仅内网防护) |
...
```bash
# 1. 备份原始配置到 openkill.config.*
save_dnsmasq_server() → uci add_list openkill.config.dnsmasq_server="<原始server>"
uci set openkill.config.dnsmasq_noresolv="$(uci get dhcp.@dnsmasq[0].noresolv)"
uci set openkill.config.dnsmasq_resolvfile="$(uci get dhcp.@dnsmasq[0].resolvfile)"
uci set openkill.config.dnsmasq_cachesize="$(uci get dhcp.@dnsmasq[0].cachesize)"
...
# 将 openkill_custom_chnroute_pass.list 中的域名加入 set
...
**恢复流程** (`revert_dnsmasq()`):
```bash
# 1. 删除 OpenKill 注入的 server
uci del dhcp.@dnsmasq[-1].server

# 2. 恢复原始 server 列表
for server in $(uci get openkill.config.dnsmasq_server); do
...
> **若表中未覆盖该错误**，应主动搜索 [OpenKill GitHub Issues](https://github.com/vernesong/OpenClash/issues) 查找是否存在相同或相似的问题，
> 优先参考高赞反应的社区回复和作者（vernesong）给出的解决方案。搜索时可使用错误关键字作为搜索词。
>
> **两类日志说明**:
> - **插件日志**（前九类）：由 OpenKill 的 Shell/Ruby/Lua 脚本产生，含 `[Info]`/`[Tip]`/`[Warning]`/`[Error]` 前缀，写入 `/tmp/openkill.log`。可在 LuCI「运行日志」页面查看。
> - **内核日志**（第十、十一类）：由 Mihomo 核心（Go 程序）产生，含 `level=debug/info/warning/error/fatal` 标记，同样写入 `/tmp/openkill.log`。`level=fatal` 会导致核心进程退出。可在 LuCI「运行日志」页面查看，或在「运行状态」页面看到 `OpenKill Start Failed` 提示。
...
| `Core Initial Configuration Timeout` (内核初始化超时) | 「运行状态」启动流程 | 核心 API 在 300 秒内未就绪 | 检查 `/tmp/openkill.log` 中核心日志；确认「覆写设置→常规」的 cn_port 未被占用 |
...
| `OpenKill Now Disabled, Need Start From Luci Page` (插件未启用) | 「运行状态」启动流程 | 插件被禁用（enable=0） | 「运行状态」页面点击启动开关 |
...
| `Config File Tested Faild` (配置文件测试失败) | 「配置订阅」更新流程 | 下载的 YAML 未通过 `clash -t` 验证 | 「配置管理」页面 Edit 检查 YAML 语法；查看 `/tmp/openkill.log` |
...
| `Pre update test failed` (更新前测试失败，3 次后) | 「版本更新」 | 插件 IPK/APK 安装测试失败 | 手动在「系统→软件包」中更新或重装 luci-app-openkill |
| `OpenKill update failed` (OpenKill 更新失败) | 「版本更新」 | 插件安装彻底失败 | 包已保存在 `/tmp/`，手动使用 `opkg install` 或 `apk add` 安装 |
...
| `OpenKill Start Failed: {msg}` (OpenKill 启动失败) | 「运行状态」 | 核心日志中出现 fatal/error 级别日志 | 查看完整错误消息；「运行日志」生成调试日志 |
...
> 日志查看：LuCI「运行日志」页面或「运行状态」页面（若启动失败会显示 `OpenKill Start Failed`）。
...
| `[CacheFile] remove invalid cache file error` | 运行缓存 | 「运行状态」停止 OpenKill，手动删除 `/etc/openkill/cache.db` 后重启 |

> **通用排查**: 在「配置管理」页面点击 **Download Run** 下载经脚本处理后的运行时配置，对比原始订阅检查 `yml_change.sh` 和覆写模块生成的 YAML 是否正确。

### 十二、DNS 泄露排查

> **核心验证方法**：在客户端执行 `nslookup www.google.com`，应返回：① DNS 服务器为 OpenWrt 路由器 IP；② 解析结果为 Fake-IP 范围地址（`198.18.x.x`）。若返回真实 IP 或上游 DNS 非路由器，说明 DNS 解析链路异常。正确链路应为：`设备 → Dnsmasq(53端口) → OpenKill(7874端口)`。
...
| `nameserver-policy` 未生效，DNS 仍走 nameserver | 「覆写设置→DNS」 | OpenKill 的「覆写设置→DNS」选项会与订阅配置的 `dns:` 段合并，可能导致预期外的 DNS 行为 | ① 在「覆写设置→DNS」启用「自定义 DNS 设置 (Custom DNS Setting)」后重新配置所有 DNS 规则；② 在「运行日志」中开启 Debug 等级观察实际 DNS 查询路径；③ 确认 `default-nameserver` 组的 DNS 服务器开启了「节点域名解析」选项 | 同上 |
| DNS 泄露（开启 IPv6 后出现） | 「覆写设置→DNS」+「IPv6 设置」 | 运营商下发的 IPv6 DNS 绕过了 OpenKill 的 DNS 劫持，直接响应客户端请求（"抢答"） | ① 在 LuCI 的「网络→DHCP/DNS→高级设置」中取消 `过滤 IPv6 AAAA 记录`；② 在 LAN 接口 DHCP 服务器 IPv6 设置中**取消「本地 IPv6 DNS 服务器」**，强制设备使用路由器 IPv4 地址进行 DNS 解析；③ DHCPv6 服务设为已禁用，RA 设为服务器模式。原理：DNS 请求走 IPv4 通道，流量走 IPv6 通道——IPv4 DNS 同样可以查询 AAAA 记录返回 IPv6 地址 | — |
...
| **自定义防火墙规则（开发者选项）不生效** | 「插件设置→开发者设置」 | 编辑后未重启或脚本语法错误 | ① 修改 `openkill_custom_firewall_rules.sh` 后需**重启 OpenKill**（不是重载防火墙）；② 用 `bash -n` 检查脚本语法；③「运行日志」生成调试日志检查是否成功执行（日志中含自定义脚本内容） | [#4005](https://github.com/vernesong/OpenClash/issues/4005) |
...
| **Cloudflare Tunnel (Cloudflared) 连接不稳定** | 路由器/内网设备 | Cloudflared 默认使用 QUIC 连接，而海外 QUIC 流量默认被 OpenKill 阻断 | ① 规则中已指定 Cloudflare Tunnel 相关域名直连；② 在 Cloudflared 启动参数中显式指定 `--protocol http2` 强制使用 HTTP/2（Docker 版：`command: [tunnel, --no-autoupdate, --protocol, http2, run, --token, ${CF_TOKEN}]`） | — |
...
| **Hysteria / Hysteria2 / TUIC 节点连接失败、断流、握手超时** | 内核日志 `level=error` | ① Linux 内核 ≥6.6 的 quic-go GSO 兼容性问题（最常见）；② Hysteria 协议对 `server`/`auth`/`tls`/`password` 字段配置敏感 | ① **优先尝试**：「插件设置→模式设置」开启**「禁用 quic-go GSO (Disable QUIC Go GSO)」**后重启 OpenKill；② 确认 YAML 中 `type: hysteria` 或 `type: hysteria2` 拼写正确、端口号正确；③ 检查节点的 `auth`/`password` 及 TLS 证书配置是否完整 | — |
...
| 元素 | 功能 | 后端操作 |
|------|------|----------|
| **启动/停止开关** | 切换核心运行状态 | 调用 `action_oc_action` → `/etc/init.d/openkill start/stop` |
| **重启按钮** | 重启核心 | 调用 `/etc/init.d/openkill restart` |
| **覆写模块按钮** | 在运行状态页弹出覆写编辑器（与菜单「服务→OpenKill→覆写设置」独立） | 调用 `editOverwrite()` → 在运行状态页弹出覆写编辑模态框 |
| **插件/核心版本** | 显示当前版本号 + 更新红点 | 已装版本: 核心执行 `/etc/openkill/core/clash_meta -v` 解析输出、插件读取 opkg/apk 包数据库，经 `/update` 端点展示; 远程最新: Lua `fetch_version_history` 拉取并缓存（内核 `/tmp/clash_last_version` / 插件 `/tmp/openkill_last_version`，Lua 侧另有 `/tmp/openkill_version_history_<branch>.json` JSON 缓存），经独立 `/last_version` 端点 (action_last_version) 获取并据此显示「新版本可用」红点，均非 `/status` 端点 |
...
| **更新配置** | 重新下载订阅并更新 | `action_update_config` → 调用 `openkill.sh` |
...
- 切换触发: `action_switch_dashboard` → `openkill_download_dashboard.sh`
...
| 操作 | 功能 | 后端 |
|------|------|------|
| **关闭链接 (Close Connect)** | 断开所有代理连接 | `openkill_history_get.sh 'close_all_conection'` |
| **重置防火墙 (Reload Firewall)** | 重新应用 iptables/nftables 规则 | `/etc/init.d/openkill reload 'manual'` |
| **清空 DNS 缓存** | 刷新 Fake-IP 和 DNS 缓存 | POST `/cache/fakeip/flush` + `/cache/dns/flush` |
| **检查更新 (Check Update)** | 同时更新插件 + 核心 + 订阅 + GEO | `openkill_update.sh 'one_key_update'` |
...
> UCI Section: `openkill` (anonymous section)
> 所有选项通过 `uci set openkill.@openkill[0].<option>=<value>` 设置
...
| `external-ui` | `/usr/share/openkill/ui` | Dashboard 路径不可更改 |
...
| `auto-redir` | **删除** | 强制移除 auto-redir（由 OpenKill 防火墙管理） |
...
- **UCI 选项**: `openkill.@openkill[0].en_mode`
- **可选值**:
...
- **UCI 选项**: `openkill.@openkill[0].stack_type`
...
- **UCI 选项**: `openkill.@openkill[0].proxy_mode`
...
- **UCI 选项**: `openkill.@openkill[0].enable_udp_proxy`
- **默认**: 1 (开启)
...
- **UCI 选项**: `openkill.@openkill[0].delay_start`
- **默认**: 0 (不延迟)
- **说明**: 开机后延迟指定秒数再启动 OpenKill

### log_size — 日志大小（KB） (Log Size)
- **UCI 选项**: `openkill.@openkill[0].log_size`
- **默认**: 1024 (1MB)
- **说明**: 核心日志文件 `/tmp/openkill.log` 的最大大小

### bypass_gateway_compatible — 旁路网关（旁路由）兼容 (Bypass Gateway Compatible)
- **UCI 选项**: `openkill.@openkill[0].bypass_gateway_compatible`
- **默认**: 0 (关闭)
- **说明**: 旁路由 (旁路网关) 模式下无法上网时尝试开启。如果设备没有 WLAN，建议禁用 LAN 接口的桥接选项

### disable_quic_go_gso — 禁用 quic-go GSO (Disable QUIC Go GSO)
- **UCI 选项**: `openkill.@openkill[0].disable_quic_go_gso`
- **默认**: 0 (关闭)
...
- **UCI 选项**: `openkill.@openkill[0].small_flash_memory`
- **默认**: 0 (关闭)
- **说明**: 将核心和 GEOIP 数据文件移到 `/tmp/etc/openkill`，节省闪存空间。路由重启后需重新下载

> **AI 行为指引 — QUIC 类节点（Hysteria / Hysteria2 / TUIC）故障排查**: 当用户报告 Hysteria、Hysteria2、TUIC 等基于 QUIC 协议的节点无法连接、频繁断流或握手超时时，AI 应先让用户生成调试日志查看核心日志中的 QUIC 相关错误。如日志含 `quic-go` / `GSO` / `timeout` 等关键字，**优先建议**用户在「插件设置→模式设置」中开启「禁用 quic-go GSO (Disable QUIC Go GSO)」选项并重启 OpenKill。这是 Linux 内核 ≥6.6 上最常见的 QUIC 兼容性问题。
> 
...
> 涉及防火墙实现细节时，查阅 [OpenKill 源码](https://github.com/vernesong/OpenClash/tree/dev) 中
> `init.d/openkill` 和 `yml_change.sh` 的相关逻辑。
> `set_firewall()` 通过 UCI `firewall.openkill` 注册为 `/var/etc/openkill.include`，由 OpenWrt firewall3/firewall4 框架加载。
...
- **UCI 选项**: `openkill.@openkill[0].router_self_proxy`
- **默认**: 1 (开启)
- **说明**: 开启后，路由器本身发出的流量也会经过代理核心。仅在规则模式下生效。关闭后流媒体增强标签页所有功能将失效
- **实现细节**: 控制 OUTPUT 链规则是否生成。开启时创建 `openkill_output` 链（fw4）或 OUTPUT 规则（fw3），将路由器自身出站流量重定向到 Clash。关闭时删除 OUTPUT 链规则，路由器自身流量走原始路由表。

### disable_udp_quic — 禁用 QUIC (Disable QUIC)
- **UCI 选项**: `openkill.@openkill[0].disable_udp_quic`
- **默认**: 1 (开启)
- **效果**: 对 UDP 443 端口的流量执行 REJECT，阻止 YouTube 等使用 QUIC 协议传输 (降级到 TCP)
- **执行方式**: 通过 iptables/nftables 规则阻断 UDP 443，排除中国大陆 IP 段
- **实现细节**: 在 `set_firewall()` 的 `openkill_mangle` 链中插入规则：`meta l4proto udp th dport 443 counter reject`。但绕过 `china_ip_route_pass` ipset 中的中国 IP（通过 `ip daddr @china_ip_route_pass counter return`）。同时配合 dnsmasq 的 ipset/nftset 标记确保国内 QUIC 不受影响。

### skip_proxy_address — 绕过服务器地址 (Skip Proxy Address)
- **UCI 选项**: `openkill.@openkill[0].skip_proxy_address`
- **默认**: 0 (关闭)
- **说明**: 绕过配置中服务器地址的代理，防止重复代理 (代理嵌套)
- **实现细节**: 开启后看门狗脚本 `openkill_watchdog.sh` 中的 `skip_proxies_address()` 函数（每 30 个看门狗周期执行一次）解析 YAML 中所有代理节点（`proxies` 和 `proxy-providers`）的 `server` 地址，域名通过 `openkill_debug_dns.lua` 调用 Mihomo 内核 API（`/dns/query`）解析为 IP 后，加入已存在的 `localnetwork` nft set（或 ipset）。由于 `openkill` 链首条规则为 `ip daddr @localnetwork counter return`，这些代理服务器 IP 自动被跳过代理，防止代理嵌套。

### common_ports — 仅允许常用端口流量 (Common Ports Proxy Mode)
- **UCI 选项**: `openkill.@openkill[0].common_ports`
- **默认**: 0 (禁用)
...
- **实现细节**: 非 0 时在 `openkill` redirect 链的端口检查规则中使用自定义端口列表（而非默认的允许所有端口）。格式 `{tcp, udp} th dport {80, 443, ...} counter redirect to proxy_port`。禁用时规则不含端口限制，所有 TCP 都重定向。

### china_ip_route — 实验性：绕过指定区域 IP (China IP Route)
- **UCI 选项**: `openkill.@openkill[0].china_ip_route`
- **可选值**:
  - `0` — 关闭
...
- **UCI 选项**: `openkill.@openkill[0].intranet_allowed`
- **默认**: 1 (开启)
- **说明**: 开启后控制面板和连接代理端口仅能从内网访问，不暴露到公网
- **Mihomo 对应**: `allow-lan: true` + `bind-address: "*"`
- **实现细节**: 双重保护——1) YAML 层面：`yml_change.sh` 设置 `allow-lan: true` + `bind-address: "*"` 使内核监听所有接口。2) 防火墙层面：创建 `openkill_wan_input` 链，匹配来自 WAN 口（非 `@localnetwork` 源 IP）的流量，REJECT 全部 7 个服务端口（`$proxy_port`/`$tproxy_port`/`$cn_port`/`$http_port`/`$socks_port`/`$mixed_port`/`$dns_port`）。关闭时 `allow-lan: false`，内核仅监听 `127.0.0.1`，同时不创建 `openkill_wan_input` 链。

### intranet_allowed_wan_name — WAN 接口名称 (WAN Interface Name)
- **UCI 选项**: `openkill.@openkill[0].intranet_allowed_wan_name`
- **说明**: 指定哪个接口被识别为 WAN。用于仅允许内网功能区分内外网
- **依赖**: `intranet_allowed=1`

### lan_interface_name — LAN 接口名称 (LAN Interface Name)
- **UCI 选项**: `openkill.@openkill[0].lan_interface_name`
- **可选值**: 系统中所有网络接口名
- **默认**: 0 (禁用)
...
- **UCI 选项**: `openkill.@openkill[0].local_network_pass`
- **存储文件**: `/etc/openkill/custom/openkill_custom_localnetwork_ipv4.list`
- **说明**: 目标地址为列表中 IP 的流量不经过核心

### chnroute_pass — 绕过指定区域 IPv4 黑名单 (Chnroute Bypassed List)
- **UCI 选项**: `openkill.@openkill[0].chnroute_pass`
- **存储文件**: `/etc/openkill/custom/openkill_custom_chnroute_pass.list`
...
- **实现细节**: 防火墙初始化阶段 `set_firewall()` 创建 `openkill_upnp` 链并在 `openkill_mangle` 链中通过 `jump openkill_upnp`（规则位置在最终 TPROXY 之前）。`upnp_exclude()` 函数读取 upnpd 租约文件（格式 `UDP:<ext_port>:<int_ip>:<int_port>`），为每条租约在 `openkill_upnp` 链中添加 `ip saddr <int_ip> <proto> sport <int_port> counter return` 规则。看门狗 `openkill_watchdog.sh` 每 30 个周期（首周期立即执行，之后每 `UPNP_INTERVAL=30` 即约 30 分钟）执行 UPNP 规则同步：① **清理过期规则**——遍历 `openkill_upnp` 链现有规则，删除租约文件中已不存在的条目；② **添加新规则**——读取租约文件，为新增的 UPnP 映射补充 RETURN 规则。
...
> 3. `openkill_custom_domain_dns.sh` 为自定义域名配置独立 DNS
>
...
> 涉及 dnsmasq 劫持实现时可查阅 [OpenKill 源码](https://github.com/vernesong/OpenClash/tree/dev) 中
> `init.d/openkill` 的 `change_dnsmasq()` 函数。对于 DNS 劫持失败的排查，首先让用户检查
> 「运行状态」页面查看 DNS 端口是否在监听。

### enable_redirect_dns — 本地 DNS 劫持 (Redirect Local DNS Setting)
- **UCI 选项**: `openkill.@openkill[0].enable_redirect_dns`
- **可选值**:
  - `0` — 禁用
...
- **实现详解**:
  - **值1 (Dnsmasq)**: `change_dnsmasq()` 函数先备份 dnsmasq 原有配置到 `openkill.config.*`，然后设置 `dhcp.@dnsmasq[0].server=127.0.0.1#<dns_port>`，`noresolv=1`，`cachesize=0`。效果：所有 LAN 客户端的 DNS 查询 → dnsmasq → 转发到 Clash DNS (7874) → Clash 根据 `enhanced-mode` 处理。
  - **值2 (防火墙)**: 创建 `openkill_dns_redirect` nftables 链，对目标端口 53 的 UDP/TCP 流量 DNAT 到 `dns_port`。同时保留 dnsmasq 处理本地 DNS 缓存。此模式允许 `lan_ac_*` 访问控制（需要 Fake-IP 模式）。
  - **恢复**: `revert_dnsmasq()` 还原所有原始 dnsmasq 配置（servers、noresolv、resolvfile、cachesize）。

### flush_dns_cache — 清空 DNS 缓存按钮 (Flush DNS Cache)
- **模板**: `openkill/flush_dns_cache`
- **功能**: 通过 POST `/cache/fakeip/flush` + `/cache/dns/flush` API 清空 Fake-IP 和 DNS 缓存

### dnsmasq_fix — Dnsmasq 修复按钮 (Dnsmasq Fix)
- **功能**: 停止 OpenKill 后 DNS 异常时使用。恢复 Dnsmasq 默认配置:
...
- **UCI 选项**: `openkill.@openkill[0].enable_custom_domain_dns_server`
- **默认**: 0
- **说明**: 为自定义域名列表指定专用 DNS 服务器，完全独立于内核 DNS 查询

### custom_domain_dns_server — 指定服务器 (Specify DNS Server)
- **UCI 选项**: `openkill.@openkill[0].custom_domain_dns_server`
- **默认**: `114.114.114.114`
- **格式**: `IP地址` 或 `IP地址#端口` (如 `127.0.0.1#5300`)

### custom_domain_dns — 自定义域名列表 (Custom Domain DNS)
- **存储文件**: `/etc/openkill/custom/openkill_custom_domain_dns.list`
- **格式**: 每行一个域名
...
> 涉及黑白名单匹配逻辑时，查阅 [OpenKill 源码](https://github.com/vernesong/OpenClash/tree/dev) 中
> `init.d/openkill` 的 `firewall_lan_ac_traffic()` 函数和 `set_firewall()` 中的 `ipset`/`nft set` 创建逻辑。
...
- **UCI 选项**: `openkill.@openkill[0].lan_ac_mode`
- **可选值**: `0` (黑名单模式) / `1` (白名单模式)
- **默认**: 0
- **说明**:
...
- **UCI 选项**: `openkill.@openkill[0].lan_ac_black_ips` (DynamicList)
- **格式**: IP 地址或 CIDR 网段
- **依赖**: `lan_ac_mode=0`
- **实现细节**: 生成 nftables set `openkill_lan_black_ip` / `openkill_lan_black_ip6`，在 `openkill` redirect 链中插入 `ip saddr @openkill_lan_black_ip counter return` 跳过规则。

### lan_ac_black_macs — 不走代理的局域网设备 Mac (LAN Bypassed Mac List)
- **UCI 选项**: `openkill.@openkill[0].lan_ac_black_macs` (DynamicList)
- **格式**: MAC 地址
...
> **生效路径**: `openkill_streaming_unlock.lua` 脚本在核心启动后被守护进程调用，
...
> 了解 GEOSITE/RULE-SET 等规则类型，并结合 OpenKill 的流媒体增强设置告知用户具体配置步骤。
> 涉及流媒体解锁检测逻辑时，查阅 [OpenKill 源码](https://github.com/vernesong/OpenClash/tree/dev) 中
> `openkill_streaming_unlock.lua` 脚本。
> 注意：`openkill_streaming_unlock.lua` 仅负责已配置服务的节点测试和自动切换，不负责 Smart 策略。
...
- **手动测试按钮**: 调用 `openkill_streaming_unlock.lua` 执行解锁测试
- **实现详解**: `openkill_streaming_unlock.lua` 是一个独立 Lua 脚本，被 `/etc/init.d/openkill` 以 `procd` 服务形式启动（与核心并列）。工作流程：
...
> 由 Mihomo 内核直接读取并提供 HTTP API。下载/切换通过 `openkill_download_dashboard.sh` 执行。
...
仪表盘版本管理通过 `action_switch_dashboard` → `openkill_download_dashboard.sh` 自动下载切换。
- **实现细节**: `yml_change.sh` 将 `cn_port`、`dashboard_password` 写入 YAML → Mihomo 内核启动 HTTP API。`openkill_download_dashboard.sh` 从 GitHub Releases 下载 Dashboard 静态文件 (yacd/metacubexd/zashboard)，解压到 `/usr/share/openkill/ui`。前端 `status.htm` 中的 JS 根据以下逻辑构造仪表盘 URL：1) 若当前浏览器 hostname 与路由器 LAN IP 相同 → `http://<lan_ip>:<cn_port>/ui/<dashboard>/`；2) 若设置了 `dashboard_forward_domain` + `dashboard_forward_port`（公网访问）→ 协议由 `dashboard_forward_ssl` 决定（0=http, 1=https），地址为 `<protocol>://<domain>:<port>/ui/<dashboard>/`；3) 其他情况 → 取当前页面协议 + LAN IP + cn_port。四个仪表盘的子路径分别为 `/ui/dashboard/`、`/ui/yacd/`、`/ui/metacubexd/`、`/ui/zashboard/`。
...
> 同时 `set_firewall()` 生成独立的 IPv6 防火墙规则链（`openkill_v6`、`openkill_mangle_v6` 等）。
> IPv6 使用单独的 TProxy/Redirect/TUN 规则链，与 IPv4 互不影响。
>
> **IPv6 DNS 核心最佳实践**：DNS 解析请求（包括 AAAA 记录查询）可以通过 IPv4 链路发送，无需 IPv6 DNS 服务器。推荐策略：① LAN 接口 DHCP 服务器中**不分配 IPv6 DNS**，强制设备用路由器 IPv4 地址做 DNS 解析；② 取消 `过滤 IPv6 AAAA 记录`（Dnsmasq 高级设置）；③ 开启 OpenKill 的「允许 IPv6 类型 DNS 解析」选项。效果：DNS 走 IPv4 通道查询（经过 OpenKill 分流），流量走 IPv6 通道传输——既防止 IPv6 DNS 抢答导致的泄露，又保证 IPv6 站点可访问
...
| 本地 IPv6 绕过地址 (Local IPv6 Network Bypassed List) | `local_network6_pass` | — | 文件: `/etc/openkill/custom/openkill_custom_localnetwork_ipv6.list` |
| 绕过指定区域 IPv6 黑名单 (Chnroute6 Bypassed List) | `chnroute6_pass` | — | 文件: `/etc/openkill/custom/openkill_custom_chnroute6_pass.list`。将列表中域名/IP 加入 `china_ip6_route_pass` nft set，不受 IPv6 绕行选项影响。依赖: `ipv6_enable=1` + `enable_redirect_dns=1` |
...
|----------|----------|----------|-------------|-------------|---------------|
| 自动更新 GeoIP MMDB 数据库 (Auto Update GeoIP MMDB) | `geo_auto_update` | `openkill_geo.sh ipdb` | `geo_update_week_time` | `geo_update_day_time` | `geo_custom_url` |
| 自动更新 GeoIP Dat 数据库 (Auto Update GeoIP Dat) | `geoip_auto_update` | `openkill_geo.sh geoip` | `geoip_update_week_time` | `geoip_update_day_time` | `geoip_custom_url` |
| 自动更新 GeoSite 数据库 (Auto Update GeoSite) | `geosite_auto_update` | `openkill_geo.sh geosite` | `geosite_update_week_time` | `geosite_update_day_time` | `geosite_custom_url` |
| 自动更新 Geo ASN 数据库 (Auto Update Geo ASN) | `geoasn_auto_update` | `openkill_geo.sh geoasn` | `geoasn_update_week_time` | `geoasn_update_day_time` | `geoasn_custom_url` |
...
- **实现细节**: 
  - **Cron 触发**: `add_cron()` 在 `openkill_geo.sh` 中为每种 GEO 类型注册 cron 任务
  - **下载流程**: `openkill_geo.sh` 使用自定义 URL（`*_custom_url`）或默认地址下载，保存到 `/etc/openkill/` 目录
...
此标签页用于设置 OpenKill 定时自动重启。

| 选项 | UCI Key | 类型 | 默认 | 说明 |
|------|---------|------|------|------|
| **定时重启 (Auto Restart)** | `auto_restart` | Flag | 0 | `0`=关闭, `1`=开启。开启后将在指定时间自动重启 OpenKill 服务 |
...
- **实现细节**: `add_cron()` 在 `/etc/crontabs/root` 中添加 `/etc/init.d/openkill restart` 的 cron 条目，按用户选择的时间和星期执行。
...
| **还原默认 (Restore Default)** | 恢复 OpenKill 为默认出厂配置（确认后跳回设置页） |
| **删除内核 (Remove Core)** | 删除所有核心二进制文件（红色危险按钮） |

- **实现细节**: 
  - 所有配置修改（编译版本/分支/Smart）通过 `/save_corever_branch` API 即时保存到 UCI。
  - 内核下载：`core_download` 路由调用 `openkill_core.sh`，前端始终传入完整下载 URL（`download_url`）。文件名 `clash-{arch}.tar.gz`（arch 即 `core_version`，已带 `linux-` 前缀）；oix 内核为 `mihomo-{arch}-{version}.gz`（走 github release / dl.dler.io）。
  - 插件下载：`openkill_update.sh`，文件名 `luci-app-openkill_{ver}_all.ipk`（opkg）或 `luci-app-openkill-{ver}.apk`（apk）。
  - **下载 URL 结构**：`package`/`core` 是仓库**分支**（非目录）。Latest（无历史 sha）：`package/{branch}/luci-app-openkill_{ver}_all.ipk`、`core/{branch}/{meta|smart}/clash-{arch}.tar.gz`（把分支名作为 ref 段）；选择历史版本（带 sha，为对应分支的 CI commit）：`{sha}/{branch}/luci-app-openkill_{ver}_all.ipk`、`{sha}/{branch}/{meta|smart}/clash-{arch}.tar.gz`。jsdelivr CDN 前缀为 `gh/vernesong/OpenKill@{ref}/...`，自定义 CDN 前缀直接拼接 raw URL。
  - **下载失败不会触发 OpenKill 重启**（`openkill_core.sh` 仅在内核真正更新成功后才置重启标志）。
  - 插件更新通过 ubus 后台安装以避免 Web 界面断连。

### 开发者选项 (Developer Settings / developer)
- **自定义防火墙规则** (`firewall_custom`): 在 LuCI 的「开发者设置」标签页中直接编辑的文本框，内容保存到 `/etc/openkill/custom/openkill_custom_firewall_rules.sh`。该脚本**不需要定义任何函数**——它是一个命令式 Shell 脚本，在所有 OpenKill 内置防火墙规则添加完毕后被直接执行（`chmod +x` 后运行）。可以在脚本中直接写 `iptables -I ...` 或 `nft add rule ...` 命令来追加自定义防火墙规则。
- **实现细节**: `set_firewall()` 函数在所有内置的 REDIRECT/TPROXY/TUN/IPv6/访问控制规则建立完毕后，检查此文件是否存在，若存在则 `chmod +x` 并执行。由于它在所有内置规则之后运行，自定义规则可以引用 OpenKill 已创建的 nftables 链和 set。每次 OpenKill 启动或防火墙重载时都会重新执行此脚本。
...
> **生效路径**：通过 `init.d/openkill` 的 `firewall_lan_ac_traffic()` 函数在代理链 **position 0** 插入规则，
...
> [OpenKill 源码](https://github.com/vernesong/OpenClash/tree/dev) 中
> `init.d/openkill` 的 `firewall_lan_ac_traffic()` 函数。
...
### 防火墙工作逻辑（基于 `init.d/openkill` → `firewall_lan_ac_traffic()` 源码）
...
|----------|----------|----------|----------|----------|-------------|
| 非 TUN（redir-host/fake-ip） | `openkill` | `openkill_output` | `openkill_mangle` | `openkill_mangle_output` | `openkill_post` |
| TUN 模式 | `openkill_mangle` | `openkill_mangle_output` | `openkill_mangle` | `openkill_mangle_output` | `openkill_post` |
| IPv6（ipv6_enable=1） | `openkill_v6` | `openkill_output_v6` | `openkill_mangle_v6` | `openkill_mangle_output_v6` | `openkill_post_v6` |
...
> UCI Section: `openkill.config_overwrite`
> 此页面用于覆写订阅配置中的特定字段，设置后通过 openkill.sh 脚本注入到生成的 YAML 中
...
- **UCI**: `openkill.@config_overwrite[0].interface_name`
- **默认**: 0 (禁用)
- **说明**: 绑定核心出站流量到指定网络接口
...
- **UCI**: `openkill.@config_overwrite[0].tolerance`
- **默认**: 0 (禁用)
...
- **UCI**: `openkill.@config_overwrite[0].urltest_address_mod`
- **默认**: 0 (禁用)
...
- **UCI**: `openkill.@config_overwrite[0].github_address_mod`
- **说明**: 通过代理/CDN 加速 GitHub 文件下载。**强烈推荐在 OpenKill 启动前就设置好此项**，因为插件和内核更新、GEO 数据库下载、Dashboard 下载均依赖 GitHub 连通性。推荐优先尝试 `https://testingcf.jsdelivr.net/`（jsDelivr 的 Cloudflare CDN），如不可用再切换其他 CDN
...
- **已知限制**: `github_address_mod` 仅对 rule-providers 和 proxy-providers 的 URL 生效。`openkill_download_dashboard.sh`（Dashboard 下载）和 `openkill_geo.sh`（GEO 更新）**不使用此变量**，这些脚本的下载 URL 为硬编码的 GitHub 直连地址。如需对 Dashboard/GEO 下载使用 CDN，可通过覆写模块的 `[General]` 段设置 `DOWNLOAD_FILE` 或使用自定义规则使相关域名直连。

### log_level — 日志等级 (Log Level)
- **UCI**: `openkill.@config_overwrite[0].log_level`
...
> 了解各字段的详细含义和用法，涉及 OpenKill 侧 DNS 覆写实现时查阅
> [OpenKill 源码](https://github.com/vernesong/OpenClash/tree/dev) 中 `yml_change.sh` 的 `yml_dns_custom()` 函数，
> 然后**结合 OpenKill 覆写模块的操作方式**告知用户如何配置，而非仅给出文档链接。

### enable_custom_dns — 自定义上游 DNS 服务器 (Custom DNS Setting)
- **UCI**: `openkill.@config_overwrite[0].enable_custom_dns`
- **默认**: 0
...
- **UCI**: `openkill.@config_overwrite[0].enable_respect_rules`
- **默认**: 0
...
- **UCI**: `openkill.@config_overwrite[0].append_wan_dns`
- **默认**: 1
...
- **实现细节**: `sys_dns_append()` 调用 `openkill_get_network.lua` 获取 WAN 口的 DNS 和网关地址，追加到 `/tmp/yaml_config.namedns.yaml`，后续被合并到 YAML `dns.nameserver`。支持 dhcp:// 协议直接从 DHCP 接口获取 DNS。

### fakeip_range — Fake-IP 范围 (IPv4) (Fake-IP Range)
- **UCI**: `openkill.@config_overwrite[0].fakeip_range`
- **默认**: 0 (禁用)
...
- **UCI**: `openkill.@config_overwrite[0].store_fakeip`
- **默认**: 1
...
- **UCI**: `openkill.@config_overwrite[0].custom_fallback_filter`
- **默认**: 0
- **说明**: 配置 DNS 防污染回退过滤器
- **配置文件**: `/etc/openkill/custom/openkill_custom_fallback_filter.yaml`
...
- **UCI**: `openkill.@config_overwrite[0].custom_fakeip_filter`
- **默认**: 0
- **仅**: Fake-IP 模式显示
- **Mihomo 对应**: `dns.fake-ip-filter`

### custom_fakeip_filter_mode — Fake-IP-Filter 模式 (Custom Fake-IP Filter Mode)
- **UCI**: `openkill.@config_overwrite[0].custom_fakeip_filter_mode`
...
- **文件**: `/etc/openkill/custom/openkill_custom_fake_filter.list`
- **格式**: 每行一个域名通配符，如 `*.lan`, `+.example.com`

### custom_name_policy — 自定义 Nameserver-Policy (Custom Name Policy)
- **UCI**: `openkill.@config_overwrite[0].custom_name_policy`
- **文件**: `/etc/openkill/custom/openkill_custom_domain_dns_policy.list`
...
- **UCI**: `openkill.@config_overwrite[0].custom_proxy_server_policy`
- **文件**: `/etc/openkill/custom/openkill_custom_proxy_server_dns_policy.list`
- **Mihomo 对应**: `dns.proxy-server-nameserver-policy`
- **说明**: 仅用于解析代理节点域名的 DNS 策略

### custom_host — 自定义 Hosts (Custom Hosts)
- **UCI**: `openkill.@config_overwrite[0].custom_host`
- **文件**: `/etc/openkill/custom/openkill_custom_hosts.list`
- **Mihomo 对应**: `dns.hosts`
...
> 涉及 OpenKill 侧实现时查阅 [OpenKill 源码](https://github.com/vernesong/OpenClash/tree/dev) 中
...
> 涉及 OpenKill 侧 Meta 选项注入实现时查阅 [OpenKill 源码](https://github.com/vernesong/OpenClash/tree/dev) 中
> `yml_change.sh` 的 sniffer/Meta 相关段，然后结合 OpenKill 的覆写设置操作路径告知用户。

### enable_tcp_concurrent — 启用 TCP 并发 (Enable Tcp Concurrent)
- **UCI**: `openkill.@config_overwrite[0].enable_tcp_concurrent`
- **默认**: 0
...
- **UCI**: `openkill.@config_overwrite[0].enable_unified_delay`
- **默认**: 0
...
- **UCI**: `openkill.@config_overwrite[0].find_process_mode`
...
- **UCI**: `openkill.@config_overwrite[0].enable_meta_sniffer`
- **默认**: 1
...
- **UCI**: `openkill.@config_overwrite[0].enable_meta_sniffer_pure_ip`
- **默认**: 1
...
- **UCI**: `openkill.@config_overwrite[0].enable_geoip_dat`
- **默认**: 0
...
- **UCI**: `openkill.@config_overwrite[0].global_ua`
...
>    了解并告知用户以下信息：
>    - **数据来源**：`component/smart/lightgbm/collector.go` — 开启 `smart_collect` 后核心会在 `/etc/openkill/` 下生成 CSV 训练数据文件（含延迟、抖动、丢包率等特征；特征工程见 `transform.go`）
...
>    - **模型部署**：训练完成后将 `Model.bin` 上传到可访问的 URL，通过 `lgbm_custom_url` 指定；或替换 `/etc/openkill/Model.bin`；模型加载与推理见 `lightgbm.go` 中的 `WeightModel`
...
- **UCI**: `openkill.@config_overwrite[0].auto_smart_switch`
- **默认**: 0
...
- **UCI**: `openkill.@config_overwrite[0].smart_policy_priority`
...
- **UCI**: `openkill.@config_overwrite[0].smart_prefer_asn`
- **默认**: 0
- **说明**: 强制查询并使用目标 ASN（自治系统号）信息，优先选择同一 ASN 的更稳定节点

### smart_enable_lgbm — 启用 LightGBM 模型 (Enable LightGBM Model)
- **UCI**: `openkill.@config_overwrite[0].smart_enable_lgbm`
- **默认**: 0
- **说明**: 使用 LightGBM 机器学习模型预测节点权重
- **实现细节**: `yml_change.sh` 配置 YAML 中的模型下载 URL 和更新间隔。`openkill_lgbm.sh` 定期下载训练好的 LightGBM 模型文件到 `/etc/openkill/Model.bin`（小闪存模式下为 `/tmp/etc/openkill/Model.bin`）。Mihomo Smart 模块加载模型后，根据节点历史延迟、抖动、丢包率等特征预测最优节点。

### smart_tolerance — Smart 组延迟容差 (Smart Group Tolerance)
- **UCI**: `openkill.@config_overwrite[0].smart_tolerance`
- **默认**: 0 (禁用)
- **可选值**: `0`(禁用) / `100` / `150` (ms)
...
- **UCI**: `openkill.@config_overwrite[0].smart_collect`
- **默认**: 0
...
- **UCI**: `openkill.@config_overwrite[0].smart_collect_size`
- **默认**: 100 (MB)
...
- **UCI**: `openkill.@config_overwrite[0].smart_collect_rate`
- **默认**: 1 (范围 0-1)
- **Mihomo YAML 映射**: `proxy-groups[].sample-rate: <rate>`
- **依赖**: `smart_collect=1`

### lgbm_auto_update — 自动更新 LightGBM 模型 (LGBM Auto Update)
- **UCI**: `openkill.@config_overwrite[0].lgbm_auto_update`
- **默认**: 0
...
- **UCI**: `openkill.@config_overwrite[0].lgbm_update_interval`
- **默认**: 72 (小时)
- **依赖**: `lgbm_auto_update=1`

### lgbm_custom_url — 自定义模型下载地址 (LGBM Custom URL)
- **UCI**: `openkill.@config_overwrite[0].lgbm_custom_url`
...
### 手动更新模型按钮
- **功能**: 点击触发 `openkill_lgbm.sh` 立即下载最新模型并显示当前模型文件时间戳
...
> **LuCI 路径**: 服务 → OpenKill → 配置管理 → 节点管理 → 编辑按钮 (groups-config)
> **注意**: groups-config 不是主菜单页面，而是通过「配置管理 → 节点 & 策略组管理」页面中的编辑按钮加载进入的隐藏子页面（controller 中注册为 `nil` 显示名）。
> **UCI Section**: `openkill.groups_config` (多条，每条对应一个策略组)
...
- **UCI**: `openkill.@config_overwrite[0].enable_rule_proxy`
- **默认**: 0
- **说明**: 开启后向配置追加 PROCESS-NAME 和 DST-PORT 规则，仅允许匹配规则的流量走代理，其余流量（如 BT/P2P）直连

### enable_custom_clash_rules — 自定义规则 (Custom Clash Rules)
- **UCI**: `openkill.@config_overwrite[0].enable_custom_clash_rules`
- **默认**: 0
- **说明**: 开启后将在运行配置的 `rules:` 段注入自定义规则文件中的内容

### custom_rules — 优先规则编辑框 (Custom Rules Priority)
- **UCI**: `openkill.@config_overwrite[0].custom_rules`
- **存储**: `/etc/openkill/custom/openkill_custom_rules.list`
- **格式**: 每行一条 Mihomo 规则，插入到规则列表顶部（优先匹配）
- **依赖**: `enable_custom_clash_rules=1`

### custom_rules_2 — 扩展规则编辑框 (Custom Rules Extended)
- **UCI**: `openkill.@config_overwrite[0].custom_rules_2`
- **存储**: `/etc/openkill/custom/openkill_custom_rules_2.list`
- **格式**: 每行一条 Mihomo 规则，插入到规则列表底部
- **依赖**: `enable_custom_clash_rules=1`

### 规则编写指南

> **当用户描述需求（如"我想让某个域名走代理"、"禁止某个 IP 走代理"）时，AI 应查阅 [Mihomo 路由规则文档](https://wiki.metacubex.one/config/rules/) 了解各规则类型的作用，涉及规则注入实现时查阅 [OpenKill 源码](https://github.com/vernesong/OpenClash/tree/dev) 中 `yml_rules_change.sh` 和 `custom_rules*.list` 的处理逻辑，然后告知用户具体的规则写法。**
...
> UCI Section: `openkill.config_subscribe` (多条)
...
> [OpenKill 源码](https://github.com/vernesong/OpenClash/tree/dev) 中 `openkill.sh` 的
...
**核心流程** (`openkill.sh` 中的 `sub_info_get()`):
...
8. 写入 `/etc/openkill/config/<name>.yaml`，标记核心需重启
...
> LuCI 路径: `服务` → `OpenKill` → `配置管理` (顺序第 80)
> UCI 映射: `openkill.config.config_path`
...
|----------|---------------|----------|------|
| 配置文件 (Config) | `config` | `/etc/openkill/config/` | `.yaml`/`.yml` 格式，上传后自动设为当前启用配置 |
| 代理集文件 (Proxy Provider File) | `proxy-provider` | `/etc/openkill/proxy_provider/` | 订阅中 `proxy-providers` 的节点文件 |
| 规则集文件 (Rule Provider File) | `rule-provider` | `/etc/openkill/rule_provider/` | 订阅中 `rule-providers` 的规则文件 |
| 内核文件 (Core File) | `clash_meta` | `/etc/openkill/core/` | 支持 `.tar.gz`/`.gz` 格式自动解压，`chmod 4755` |
| 备份恢复 | `backup-file` | 恢复到 `/etc/config/openkill` | 上传备份并恢复 UCI 配置 |
...
| **Download Run** (下载运行配置) | 下载运行时配置 | 下载 `/etc/openkill/<name>`（经脚本处理后的实际运行配置） |
| **Remove** (移除) | 删除配置 | 删除 YAML 文件 + 历史缓存 `/etc/openkill/history/<name>.db` + 运行时配置，自动切换到其他配置 |

## 5.3 配置文件编辑器

双栏 YAML 编辑器：
- **左栏 (可编辑)**: 读写当前 `config_path` 指向的配置，保存时自动 `\r\n` → `\n` 转换
- **右栏 (只读)**: 展示运行时配置 `/etc/openkill/<name>` 或默认模板 `/usr/share/openkill/res/default.yaml`
...
> LuCI 路径: `服务` → `OpenKill` → `运行日志` (顺序第 90)
> UCI 映射: `openkill.openkill.clog`

## 实现总览

运行日志页面是一个双标签页的日志查看器。页面布局包含以下区域：

**标签页 1 — Plugin Logs** (默认激活)：展示 OpenKill 插件自身日志（Shell/Ruby/Lua 脚本输出），通过 XHR 轮询 (`/refresh_log`) 每秒刷新。
...
**标签页 3 — Debug Logs** (可切换)：展示插件的调试日志，内容由 `openkill_debug.sh` 生成，包含系统信息、依赖包检查、内核运行状态、插件设置、覆写模块设置、自定义规则文件内容、当前 Mihomo YAML 配置、自定义覆写/防火墙脚本内容、完整的 iptables-save dump、完整的 nftables 规则、ipset 状态、路由表、TUN 设备状态、端口占用、DNS 解析测试、网络连通性测试、最近运行日志、活动连接列表及隐私处理等。
...
**OpenKill|Mihomo 日志来源**: OpenKill 日志由后端将 UCI `clog` 字段内容写入 CodeMirror 日志编辑器。内核日志通过 WebSocket 实时推送到前端 `textarea#core_log`。

**调试日志来源|实现细节**: `openkill_debug.sh` 使用文件锁防止并发，收集以下 20+ 个章节并输出到 `/tmp/openkill_debug.log`：
  ...
  5) 覆写模块设置（`uci show openkill.@overwrite[0]`）
  6) 自定义规则文件内容
...
**附加组件**: 页面同时加载 `openkill/toolbar_show`（**配置切换工具栏**：下拉选择当前配置文件 + Switch 按钮）和 `openkill/config_editor`（**页面内嵌 CodeMirror 编辑器**，预加载 CodeMirror CSS/JS 资源并通过全局 `merge_editor()` 函数对外开放合并视图功能，非日志渲染用途，仅作文件编辑功能复用）。

> **注意**: 如需修改 OpenKill 自身日志级别，请在「覆写设置 → 常规设置」中调整 `log_level`。内核日志标签页内可直接切换内核日志等级。
...
| 3. TCP 代理链 | `nft list chain inet fw4 openkill 2>/dev/null \| head -20` | 🟢 | 含 `redirect to 7892` | 链不存在→`/etc/init.d/openkill reload` |
| 4. 策略路由 | `ip rule show \| grep 0x162` | 🟢 | `fwmark 0x162 lookup 0x162` | TUN 模式必须；无→重启核心 |
| 5. 错误日志 | `tail -30 /tmp/openkill.log \| grep -E 'level=(error\|fatal)'` | 🟢 | 无输出 | 有→对照「日志与错误信息速查」 |
...
| 2. DNS 劫持 | `nft list chain inet fw4 dstnat 2>/dev/null \| grep 'OpenKill DNS'` | 🟢 | 含 `redirect` 规则 | 无→检查 `enable_redirect_dns` UCI |
...
| 步骤 | 命令 | 安全 | 期望输出 | 异常处理 |
|------|------|------|----------|----------|
| 1. AC 模式 | `uci get openkill.@openkill[0].lan_ac_mode` | 🟢 | `0`(黑名单) 或 `1`(白名单) | 确认与用户预期一致 |
...
| 4. 规则确认 | `nft list chain inet fw4 openkill \| grep -E 'saddr\|ether saddr'` | 🟢 | AC 规则在代理规则之前 | 顺序异常→重载防火墙 |
...
| 5. 错误日志 | `tail -30 /tmp/openkill.log \| grep -iE 'error\|timeout\|refused\|reset\|fatal'` | 🟢 | 无输出 | 有→对照「日志与错误信息速查」 |

### 7.1.6 配置/启动失败

| 步骤 | 命令 | 安全 | 期望输出 | 异常处理 |
|------|------|------|----------|----------|
| 1. 启动日志 | `tail -30 /tmp/openkill_start.log` | 🟢 | 含 `Start Successful` | `Core Start Failed`→跳步骤3 |
| 2. UCI enable | `uci get openkill.@openkill[0].enable` | 🟢 | `1` | `0`→插件被禁用 |
| 3. YAML 验证 | `/etc/openkill/clash -t -d /etc/openkill -f $(uci get openkill.@openkill[0].config_path)` | 🟡 | `configuration is ok` | 报错→对照「日志与错误信息速查」 |
...
| 6. 调试日志 | `/usr/share/openkill/openkill_debug.sh` | 🟡 | 生成 `/tmp/openkill_debug.log` | 日志含 `#===== 依赖检查 =====#` 段 |
...
|------|------|
| `pidof clash` | 核心是否运行 |
| `/etc/openkill/clash -v` | 核心版本 |
| `uci show openkill \| grep <key>` | 查特定 UCI 配置 |
| `tail -50 /tmp/openkill.log` | 运行日志 |
| `nft list chain inet fw4 openkill` | TCP 透明代理规则链 |
| `nft list chain inet fw4 openkill_mangle` | UDP TPROXY 规则链 |
| `nft list chain inet fw4 dstnat \| grep 'OpenKill DNS'` | DNS 劫持规则 |
...
| 命令 | 用途 | 副作用 |
|------|------|--------|
| `/etc/init.d/openkill reload` | 重载防火墙 | 不影响已有连接 |
| `/usr/share/openkill/openkill_debug.sh` | 生成调试日志 | 写 `/tmp/openkill_debug.log` |
| `/usr/share/openkill/openkill_geo.sh <type>` | 更新 GEO | 替换文件；type=`ipdb\|geoip\|geosite\|geoasn\|all` |
| `/usr/share/openkill/openkill_chnroute.sh` | 更新大陆路由 | 替换 ipset/nft set |
| `/usr/share/openkill/openkill_history_get.sh close_all_conection` | 断开所有连接 | 中断活跃代理连接 |
...
| 命令 | 用途 | 风险 |
|------|------|------|
| `/etc/init.d/openkill restart` | 重启核心 | 全网断流 3-5 秒 |
| `/etc/init.d/openkill stop` | 停止核心 | 全网断流 |
| `/usr/share/openkill/openkill.sh <name>` | 更新订阅 | 替换配置+自动重启 |
| `/usr/share/openkill/openkill_core.sh Meta` | 更新核心 | 下载+替换+重启 |
| `/usr/share/openkill/openkill_update.sh` | 更新插件 | 替换 IPK |
| `/usr/share/openkill/openkill_update.sh one_key_update` | 一键更新 | 核心+插件+订阅+GEO+重启 |
| `uci set openkill.@openkill[0].<key>=<value> && uci commit openkill` | 修改 UCI | 可能打断正常服务 |
...
curl -s http://127.0.0.1/cgi-bin/luci/admin/services/openkill/status

# 当前代理模式 (rule/global/direct)
curl -s http://127.0.0.1/cgi-bin/luci/admin/services/openkill/rule_mode

# 当前运行模式 (redir-host/fake-ip/redir-host-tun 等)
curl -s http://127.0.0.1/cgi-bin/luci/admin/services/openkill/get_run_mode

# 实时流量统计（上下行速率、连接数、CPU、内存）
curl -s http://127.0.0.1/cgi-bin/luci/admin/services/openkill/toolbar_show
...
curl -s http://127.0.0.1/cgi-bin/luci/admin/services/openkill/update

# 远程最新版本（corelv 远程最新核心 / oplv 远程最新插件，首次请求会同步拉取缓存）
curl -s http://127.0.0.1/cgi-bin/luci/admin/services/openkill/last_version

# 配置文件列表及当前使用的配置
curl -s http://127.0.0.1/cgi-bin/luci/admin/services/openkill/config_name

# 混合代理端口和认证信息
curl -s http://127.0.0.1/cgi-bin/luci/admin/services/openkill/proxy_info

# 订阅流量/到期信息（替换 <配置名>）
curl -s 'http://127.0.0.1/cgi-bin/luci/admin/services/openkill/sub_info_get?filename=<配置名>'

# 快捷设置状态（sniffer/respected_rules/china_ip_route/stream_unlock）
curl -s http://127.0.0.1/cgi-bin/luci/admin/services/openkill/oc_settings

# 最后一行启动日志
curl -s http://127.0.0.1/cgi-bin/luci/admin/services/openkill/startlog

# 核心文件是否存在
curl -s http://127.0.0.1/cgi-bin/luci/admin/services/openkill/check_core

# 多源出口 IP 查询（UpaiYun/IPIP/IP.SB/IPIFY 并行）
curl -s http://127.0.0.1/cgi-bin/luci/admin/services/openkill/myip_check

# 网站可达性检测（替换域名，返回延迟 ms）
curl -s 'http://127.0.0.1/cgi-bin/luci/admin/services/openkill/website_check?domain=www.google.com'
```
...
# --- 服务控制 ---
# 启动
curl -s -X POST -d 'action=start' http://127.0.0.1/cgi-bin/luci/admin/services/openkill/action
# 停止
curl -s -X POST -d 'action=stop' http://127.0.0.1/cgi-bin/luci/admin/services/openkill/action
# 重启
curl -s -X POST -d 'action=restart' http://127.0.0.1/cgi-bin/luci/admin/services/openkill/action

# --- 热切换（无需重启核心） ---
# 切换代理模式为 rule
curl -s -X POST -d 'rule_mode=rule' http://127.0.0.1/cgi-bin/luci/admin/services/openkill/switch_rule_mode
# 切换代理模式为 global
curl -s -X POST -d 'rule_mode=global' http://127.0.0.1/cgi-bin/luci/admin/services/openkill/switch_rule_mode
# 切换代理模式为 direct
curl -s -X POST -d 'rule_mode=direct' http://127.0.0.1/cgi-bin/luci/admin/services/openkill/switch_rule_mode

# 切换日志级别
curl -s -X POST -d 'log_level=debug' http://127.0.0.1/cgi-bin/luci/admin/services/openkill/switch_log
curl -s -X POST -d 'log_level=info' http://127.0.0.1/cgi-bin/luci/admin/services/openkill/switch_log
curl -s -X POST -d 'log_level=warning' http://127.0.0.1/cgi-bin/luci/admin/services/openkill/switch_log

# --- 缓存操作 ---
# 清空 DNS+Fake-IP 缓存
curl -s -X POST http://127.0.0.1/cgi-bin/luci/admin/services/openkill/flush_dns_cache
# 清空 Smart 缓存
curl -s -X POST http://127.0.0.1/cgi-bin/luci/admin/services/openkill/flush_smart_cache

# --- 防火墙与连接 ---
# 重载防火墙规则
curl -s -X POST http://127.0.0.1/cgi-bin/luci/admin/services/openkill/reload_firewall
# 断开所有代理连接
curl -s -X POST http://127.0.0.1/cgi-bin/luci/admin/services/openkill/close_all_connection

# --- 快捷设置切换 ---
# 启用域名嗅探
curl -s -X POST -d 'setting=meta_sniffer&value=1' http://127.0.0.1/cgi-bin/luci/admin/services/openkill/switch_oc_setting
# 启用 DNS 尊重规则
curl -s -X POST -d 'setting=respect_rules&value=1' http://127.0.0.1/cgi-bin/luci/admin/services/openkill/switch_oc_setting
# 切换区域绕行：0=关闭 1=绕过大陆 2=绕过海外
curl -s -X POST -d 'setting=oversea&value=1' http://127.0.0.1/cgi-bin/luci/admin/services/openkill/switch_oc_setting
# 启用流媒体解锁
curl -s -X POST -d 'setting=stream_unlock&value=1' http://127.0.0.1/cgi-bin/luci/admin/services/openkill/switch_oc_setting

# --- 配置与订阅 ---
# 切换配置文件（替换 <文件名.yaml>，会自动重启核心）
curl -s -X POST -d 'config_file=<文件名.yaml>' http://127.0.0.1/cgi-bin/luci/admin/services/openkill/switch_config
# 更新指定订阅配置（替换 <配置名>）
curl -s -X POST -d 'filename=<配置名>' http://127.0.0.1/cgi-bin/luci/admin/services/openkill/update_config

# --- 更新操作 ---
# 更新核心（按 uci 配置构建下载 URL，无完整 URL）
curl -s -X POST -d 'core_type=Meta' http://127.0.0.1/cgi-bin/luci/admin/services/openkill/coreupdate
# 更新插件（无参=只升级插件）
curl -s -X POST http://127.0.0.1/cgi-bin/luci/admin/services/openkill/opupdate
# 一键更新（核心+插件+订阅+GEO）
curl -s -X POST http://127.0.0.1/cgi-bin/luci/admin/services/openkill/one_key_update

# --- 生成 PAC 文件 ---
curl -s -X POST http://127.0.0.1/cgi-bin/luci/admin/services/openkill/generate_pac
```
...
```bash
# 连接诊断（测试指定域名/IP 可达性）
curl -s 'http://127.0.0.1/cgi-bin/luci/admin/services/openkill/diag_connection?addr=www.google.com'

# DNS 诊断（测试指定域名的 DNS 解析链路）
curl -s 'http://127.0.0.1/cgi-bin/luci/admin/services/openkill/diag_dns?addr=www.google.com'

# 生成并返回完整调试日志（等同于 openkill_debug.sh）
curl -s http://127.0.0.1/cgi-bin/luci/admin/services/openkill/gen_debug_logs

# 返回已有的调试日志（不重新生成）
curl -s http://127.0.0.1/cgi-bin/luci/admin/services/openkill/get_debug_logs

# 手动流媒体解锁测试（替换 <服务名>，如 netflix/disney/hbo_max 等）
curl -s 'http://127.0.0.1/cgi-bin/luci/admin/services/openkill/manual_stream_unlock_test?type=<服务名>'
```
...
curl -s -X PUT -H 'Content-Type: application/json' \
  -d '{"path":"/etc/openkill/<配置文件名>.yaml"}' \
...
> 所有脚本路径以 `/usr/share/openkill/` 为前缀，需在路由器终端执行。

### 用户可直调的脚本

```bash
# --- 诊断 ---
# 生成完整调试日志（输出到 /tmp/openkill_debug.log，含依赖检查、防火墙规则、配置等 20+ 章节）
/usr/share/openkill/openkill_debug.sh

# --- GEO 数据库更新 ---
# 更新 GeoIP MMDB 数据库
/usr/share/openkill/openkill_geo.sh ipdb
# 更新 GeoIP Dat 数据库
/usr/share/openkill/openkill_geo.sh geoip
# 更新 GeoSite 数据库
/usr/share/openkill/openkill_geo.sh geosite
# 更新 Geo ASN 数据库
/usr/share/openkill/openkill_geo.sh geoasn
# 更新全部 GEO 数据库
/usr/share/openkill/openkill_geo.sh all
...
/usr/share/openkill/openkill_version.sh
# 使用 CDN 加速检查
/usr/share/openkill/openkill_version.sh https://testingcf.jsdelivr.net/

# --- 仪表盘下载 ---
# 下载 Zashboard（推荐）
/usr/share/openkill/openkill_download_dashboard.sh Zashboard Official
# 下载 Metacubexd
/usr/share/openkill/openkill_download_dashboard.sh Metacubexd Official
# 下载 Yacd
/usr/share/openkill/openkill_download_dashboard.sh Yacd Official
# 下载 Yacd（Meta 分支）
/usr/share/openkill/openkill_download_dashboard.sh Yacd Meta

# --- LightGBM 模型 ---
# 手动下载最新 LightGBM 模型
/usr/share/openkill/openkill_lgbm.sh

# --- 连接管理 ---
# 关闭所有代理连接
/usr/share/openkill/openkill_history_get.sh close_all_conection

# --- 服务控制 ---
# 重载防火墙规则（不重启核心，不影响已有连接）
/etc/init.d/openkill reload

# --- 🔴 以下命令会触发重启或替换文件，需谨慎 ---

# 更新单个订阅配置（替换 <配置文件名>，不含路径和扩展名）
/usr/share/openkill/openkill.sh <配置文件名>

# 下载并替换 Meta 核心（使用默认 GitHub 地址）
/usr/share/openkill/openkill_core.sh Meta
# 下载并替换 Meta 核心（使用 CDN 加速）
/usr/share/openkill/openkill_core.sh Meta https://testingcf.jsdelivr.net/

# 更新 luci-app-openkill 插件
/usr/share/openkill/openkill_update.sh
# 一键更新（核心+插件+订阅+GEO，使用 CDN 加速）
/usr/share/openkill/openkill_update.sh one_key_update https://testingcf.jsdelivr.net/

# 重启核心（全网断流 3-5 秒）
/etc/init.d/openkill restart
# 停止核心（全网断流）
...
| `openkill_watchdog.sh` | init.d | 核心存活+防火墙完整性检查 |
| `openkill_custom_domain_dns.sh` | init.d | 自定义域名 DNS |
| `openkill_debug_dns.lua` | Web UI | DNS 解析测试 |
| `openkill_debug_getcon.lua` | Web UI | 活动连接获取 |
| `openkill_streaming_unlock.lua` | 看门狗 | 流媒体解锁切换 |
| `openkill_sub_parser.lua` | 看门狗 | 订阅格式解析 |
...
> 覆写模块 (Overwrite Module) 是 OpenKill 的高级自定义功能
> **主入口（用户询问"怎么用"时优先讲这个）**: 运行状态页顶部**「覆写模块」按钮**（`id="edit_overwrite"`，与启动/停止开关并列，调用 `editOverwrite()`）——点击弹出覆写编辑器窗口，创建/编辑/删除/启停覆写模块都在这一个窗口内完成。
> 次要入口: 菜单 `服务→OpenKill→覆写设置`（独立 CBI 页面，配置的是内置覆写选项，与「覆写模块」文件编辑是不同入口，见第三部分）
> UCI Section: `openkill.config_overwrite` (支持多条，按 order 排序)
> 覆写文件存储: `/etc/openkill/overwrite/<名称>` (本地) 或通过 HTTP 远程拉取；内置固定文件 `/etc/openkill/custom/openkill_custom_overwrite.sh`
...
>    ③ 卡片栏「+」新建覆写模块（File / Subscribe 两种方式），另有内置固定 `openkill_custom_overwrite.sh`；
>    ④ 选卡片 → 主编辑器按 INI 三段格式编辑 → Save 落盘 `/etc/openkill/overwrite/<名称>`；
...
>    - 覆写执行机制/排序/脚本逻辑 → [OpenKill 源码](https://github.com/vernesong/OpenClash/tree/dev) 中 `init.d/openkill` 的 `overwrite_file()` 函数和 `/tmp/yaml_overwrite.sh` 生成逻辑
>
...
>    **警告：覆盖硬编码项可能导致 OpenKill 工作异常**（如 `allow-lan: false` 会使局域网设备无法使用代理端口），提醒用户谨慎操作。
...
- 触发：`editOverwrite()` → `ConfigEditor.showOverwrite()`（`config_edit.htm`），复用配置编辑器模态框。与菜单「服务→OpenKill→覆写设置」CBI 页面是**两个独立入口**。
...
- 内置一张始终存在的 **`openkill_custom_overwrite.sh`** 卡片（文件名固定，不可改名，存于 `/etc/openkill/custom/`）。
...
- 点 Save → POST `/config_file_save`（`config_file` + `content`），后端仅允许写入 `/etc/openkill/overwrite/<名称>` 或 `/etc/openkill/custom/openkill_custom_overwrite.sh`（其它路径拒绝）。

**⑤ 原理（生效机制）**
- 覆写文件落盘 `/etc/openkill/overwrite/<名称>`，并注册到 UCI `openkill.config_overwrite`（按 order 排序、config 匹配当前配置）。
- 重启 OpenKill 时 `overwrite_file()`（`init.d/openkill`）按段头解析：`[General]` 提前写入 UCI（影响 `yml_change.sh` 行为）；`[Overwrite]`/`[YAML]` 生成 `/tmp/yaml_overwrite.sh`，在 `yml_change.sh`/`yml_rules_change.sh` **之后**执行 → 深度合并/覆盖订阅与 LuCI 输出（含硬编码项，覆盖需谨慎）。
...
**核心机制**: OpenKill 的覆写模块分两个阶段执行（均在 `/etc/init.d/openkill start_service` 流程中）：
...
3. 读取 `/etc/openkill/overwrite/<名称>` 文件内容
4. 解析 `[General]` 段 → 将键值对写入 UCI `openkill.@overwrite[0]`（如 `EN_MODE`、`DNS_PORT` 等），供后续 `yml_change.sh` 读取
...
> **⚠️ 强制要求**：覆写文件**必须包含至少一个段头**（`[General]`、`[Overwrite]`、`[YAML]` 之一），否则所有内容将被忽略，覆写模块不会生效。这是因为 `overwrite_file()` 函数（`/etc/init.d/openkill`）按段头解析文件内容——所有标志位 `in_general`/`in_overwrite`/`in_yaml` 初始为 `0`，仅在遇到对应段头时才设为 `1`。段头之前、之后无段头的内容均被跳过。空行和以 `#`/`;` 开头的注释行会被安全忽略，不影响段头解析。
...
| 类型 | UCI `type` 值 | 说明 |
|------|--------------|------|
| **本地文件** | `file` | 读取 `/etc/openkill/overwrite/<名称>` |
| **远程模块** | `http` | 从 URL 下载到 `/etc/openkill/overwrite/<名称>`，支持 cron 定时更新 |
...
- `all` — 对所有配置文件生效
- `/etc/openkill/config/xxx.yaml` — 仅对该配置文件生效
...
[Overwrite]
# 追加规则文件
ruby_arr_head_add_file "$CONFIG_FILE" "rules" "/etc/openkill/custom/openkill_custom_rules.list"
...
```ini
[General]
CONFIG_FILE = /etc/openkill/config/my_custom.yaml
...
```ini
[General]
DOWNLOAD_FILE = url=https://example.com/rules.yaml, path=/etc/openkill/rule_provider/custom_rules.yaml, ua=clash-verge/v2.4.5, cron=0 2 * * *
```

## 8.6 自定义覆写脚本（旧方式，兼容保留）

**文件**: `/etc/openkill/custom/openkill_custom_overwrite.sh`
...
if [ -f "$CFG_FILE" ]; then
    ruby_arr_head_add_file "$CFG_FILE" "rules" "/etc/openkill/custom/openkill_custom_rules.list"
fi
```

## 8.7 UCI 覆写条目结构速查

每个 `config_overwrite` 条目（对应 `/etc/config/openkill` 中 `config config_overwrite` 段）的 UCI 字段：

| UCI Key | 类型 | 默认 | 说明 |
|---------|------|------|------|
| `name` | string | *(必填)* | 唯一标识，对应 `/etc/openkill/overwrite/<name>` 覆写文件名 |
| `enable` | bool | `0` | `1`=启用该覆写条目 |
| `type` | string | `file` | `file`=本地文件；`http`=远程下载（需配置 `url`/`update_days`/`update_hour`） |
| `url` | string | *(空)* | `type=http` 时的下载地址 |
| `config` | ListValue | *(空)* | 目标配置文件列表。`all`=应用到所有配置；或指定具体路径如 `/etc/openkill/config/xx.yaml`。**为空则永不匹配，覆写不生效** |
...
- `type=http` 时，`init.d` 的 `add_overwrite_cron()` 注册 cron 任务定时从 `url` 下载覆写文件到 `/etc/openkill/overwrite/<name>`
...
> 覆写文件的 `[General]` 段中可设置以下 key（大小写不敏感），写入 UCI `openkill.@overwrite[0]`。
> 来源：`init.d/openkill` → `overwrite_file()` → `allowed_keys_types` 列表。
...
> **强制规则：当用户询问本文档未覆盖的 Mihomo/OpenKill 配置或实现细节时，禁止自行猜测或编造回答。
> AI 必须主动查询外部资源获取准确信息后回复用户。**

本文档仅覆盖 OpenKill LuCI 插件 UI 中可直接配置的选项及其实现。当遇到本文档未覆盖的问题时，AI 必须**主动**使用以下资源查询答案，而非让用户自己去查阅文档：
...
| 3 | **OpenKill Issues** `https://github.com/vernesong/OpenClash/issues` | 使用 `fetch_webpage` 打开 Issue 搜索页面或具体 Issue 页面 | 插件侧功能异常/报错（配置/订阅/防火墙/UI等），搜索已知问题和社区方案（优先作者 vernesong 回复和高赞回答） |
...
| 5 | **OpenKill 源码** `github.com/vernesong/OpenClash/tree/dev` | 使用 `github_text_search` 搜索对应脚本/函数 | 需要了解插件侧的实现逻辑、UCI 到 YAML 的转换细节 |
