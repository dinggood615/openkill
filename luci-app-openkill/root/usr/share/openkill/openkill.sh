...
del_lock() {
   ...
   rm -rf "/tmp/lock/openkill_subs.lock" 2>/dev/null
}
...
urlencode() {
   if [ "$#" -eq 1 ]; then
      echo "$(/usr/share/openkill/openkill_urlencode.lua "$1")"
   fi
}

kill_streaming_unlock() {
   streaming_unlock_pids=$(unify_ps_pids "openkill_streaming_unlock.lua")
...
config_su_check()
{
   ...
   if [ -z "$CONFIG_PATH" ]; then
      uci -q set openkill.config.config_path="$CONFIG_FILE"
      uci commit openkill
	fi
...
sub_info_get()
{
   ...
   CONFIG_FILE="/etc/openkill/config/$name.yaml"
   ...
   if [ "$sub_convert" -eq 0 ]; then
      ...
      if [ "$template" != "0" ]; then
         template_path=$(grep "^$template," /usr/share/openkill/res/sub_ini.list |awk -F ',' '{print $3}' 2>/dev/null)
      else
