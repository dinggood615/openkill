...
yml_other_set()
{
   ...
   begin
      ...
      thread_pool << Thread.new{
         ...
         begin
            if $3 == 1 then
               if system('strings /etc/openkill/GeoSite.dat /etc/openkill/GeoSite.dat |grep -i category-public-tracker >/dev/null 2>&1') then
         ...
         begin
            CUSTOM_RULE = File::exist?('/etc/openkill/custom/openkill_custom_rules.list') ? YAML.load_file('/etc/openkill/custom/openkill_custom_rules.list') : {};
            CUSTOM_RULE_2 = File::exist?('/etc/openkill/custom/openkill_custom_rules_2.list') ? YAML.load_file('/etc/openkill/custom/openkill_custom_rules_2.list') : {};
         ...
         begin
            if $1 == 1 then
               custom_files = [
                  { file: '/etc/openkill/custom/openkill_custom_rules.list', position: 'top' },
                  { file: '/etc/openkill/custom/openkill_custom_rules_2.list', position: 'bottom' }
               ];
