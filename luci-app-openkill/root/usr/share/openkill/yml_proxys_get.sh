...
set_lock() {
   exec 875>"/tmp/lock/openkill_proxies_get.lock" 2>/dev/null
   flock -x 875 2>/dev/null
}

del_lock() {
   flock -u 875 2>/dev/null
   rm -rf "/tmp/lock/openkill_proxies_get.lock"
}
...
ruby -ryaml -rYAML -I "/usr/share/openkill" -E UTF-8 -e "
   ...
   Value['proxy-providers'].each_with_index do |(x,y), index|
      uci_name_tmp_prv << %x{uci -q add openkill proxy-provider 2>&1}.chomp;
      queue.push(nil)
      threads_pr << Thread.new {
         begin
            YAML.LOG('Start Getting【${CONFIG_NAME} - ' + y['type'].to_s + ' - ' + x.to_s + '】Proxy-provider Setting...');
            #代理集不存在时添加新代理集
            uci_set='set openkill.' + uci_name_tmp_prv[index] + '.';
            uci_add='add_list openkill.' + uci_name_tmp_prv[index] + '.';
            uci_del='uci -q delete openkill.' + uci_name_tmp_prv[index] + '.';
   ...
   Value['proxies'].each_with_index do |x, index|
      uci_name_tmp << %x{uci -q add openkill servers 2>&1}.chomp;
      queue.push(nil)
      threads_pr << Thread.new {
         begin
            ...
            uci_del='uci -q delete openkill.' + uci_name_tmp[index] + '.';
            uci_commands << uci_set + 'enabled=1';
            uci_commands << uci_set + 'manual=0';
            uci_commands << uci_set + 'config=\"$CONFIG_NAME\"';
