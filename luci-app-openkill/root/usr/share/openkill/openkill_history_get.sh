#!/bin/sh
. /lib/functions.sh
. /usr/share/openkill/openkill_ps.sh
. /usr/share/openkill/uci.sh
. /usr/share/openkill/runtime.sh

set_lock() {
   exec 881>"/tmp/lock/openkill_history_get.lock" 2>/dev/null
   flock -x 881 2>/dev/null
}

del_lock() {
   flock -u 881 2>/dev/null
   rm -rf "/tmp/lock/openkill_history_get.lock" 2>/dev/null
}

close_all_conection() {
   SECRET=$(uci_get_config "dashboard_password")
   lan_interface_name=$(uci_get_config "lan_interface_name" || echo "0")
   if [ "$lan_interface_name" = "0" ]; then
      LAN_IP=$(uci -q get network.lan.ipaddr |awk -F '/' '{print $1}' 2>/dev/null || ip address show $(uci -q -p /tmp/state get network.lan.ifname || uci -q -p /tmp/state get network.lan.device) | grep -w "inet"  2>/dev/null |grep -Eo 'inet [0-9\.]+' | awk '{print $2}' |head -1 || ip addr show 2>/dev/null | grep -w 'inet' | grep 'global' | grep 'brd' | grep -Eo 'inet [0-9\.]+' | awk '{print $2}' | head -n 1)
   else
      LAN_IP=$(ip address show $lan_interface_name | grep -w "inet"  2>/dev/null |grep -Eo 'inet [0-9\.]+' | awk '{print $2}' |head -1)
   fi
   PORT=$(uci_get_config "cn_port")
   curl -m 2 -H "Authorization: Bearer ${SECRET}" -H "Content-Type:application/json" -X DELETE http://"$LAN_IP":"$PORT"/connections >/dev/null 2>&1
}

set_lock

if [ "$1" = "close_all_conection" ]; then
   close_all_conection
   del_lock
   exit 0
fi

CONFIG_FILE=$(unify_ps_cfgname)
CONFIG_NAME=$(echo "$CONFIG_FILE" |awk -F '/' '{print $4}' 2>/dev/null)
small_flash_memory=$(uci_get_config "small_flash_memory")
HISTORY_PATH_OLD="/etc/openkill/history/${CONFIG_NAME%.*}"
HISTORY_PATH="/etc/openkill/history/${CONFIG_NAME%.*}.db"
core_version=$(uci_get_config "core_version" || echo 0)
CACHE_PATH_OLD="/etc/openkill/.cache"
source "/etc/openwrt_release"

if [ -z "$CONFIG_FILE" ] || [ ! -f "$CONFIG_FILE" ]; then
   CONFIG_FILE=$(uci_get_config "config_path")
   CONFIG_NAME=$(echo "$CONFIG_FILE" |awk -F '/' '{print $5}' 2>/dev/null)
   HISTORY_PATH_OLD="/etc/openkill/history/${CONFIG_NAME%.*}"
   HISTORY_PATH="/etc/openkill/history/${CONFIG_NAME%.*}.db"
fi

if openkill_core_process_present && [ -f "$CONFIG_FILE" ]; then
   # `opkg status`/`apk list` are read-only.  Never delete the package-manager
   # lock here: this script also runs from a package prerm while opkg itself is
   # upgrading OpenKill, and removing the lock corrupts that transaction's
   # cleanup path.
   if [ "$small_flash_memory" == "1" ] || [ -n "$(echo $core_version |grep mips)" ] || [ -n "$(echo $DISTRIB_ARCH |grep mips)" ] || [ -n "$(opkg status libc 2>/dev/null |grep 'Architecture' |awk -F ': ' '{print $2}' |grep mips)" ] || [ -n "$(apk list libc 2>/dev/null |grep mips)" ]; then
      CACHE_PATH="/tmp/etc/openkill/cache.db"
      if [ -f "$CACHE_PATH" ]; then
         cmp -s "$CACHE_PATH" "$HISTORY_PATH"
         if [ "$?" -ne "0" ]; then
            cp "$CACHE_PATH" "$HISTORY_PATH" 2>/dev/null
         fi
      fi
   fi
   if [ -f "$CACHE_PATH_OLD" ]; then
      cmp -s "$CACHE_PATH_OLD" "$HISTORY_PATH_OLD"
      if [ "$?" -ne "0" ]; then
         cp "$CACHE_PATH_OLD" "$HISTORY_PATH_OLD" 2>/dev/null
      fi
   fi
fi

del_lock
