...
uci_get_config() {
    local key="$1"
    uci -q get openkill.@overwrite[0]."$key" || uci -q get openkill.config."$key"
}
...
uci_set_age_keys_by_name() {
  ...
  if [ -z "$target_section" ]; then
    target_section=$(uci -q add openkill config_age_secret)
  fi

  [ -n "$target_section" ] || return 1

  uci -q set openkill."$target_section".name="$name"
  [ -n "$secret" ] && uci -q set openkill."$target_section".secret="$secret"
  [ -n "$public" ] && uci -q set openkill."$target_section".public="$public"
