...
if [ "$1" = "fake-ip" ] && [ "$enable_redirect_dns" != "2" ]; then
   ...
   if [ "$china_ip_route" != "0" ]; then
      process_pass_list "/etc/openkill/custom/openkill_custom_chnroute_pass.list"
   fi
   if [ "$china_ip6_route" != "0" ]; then
      process_pass_list "/etc/openkill/custom/openkill_custom_chnroute6_pass.list"
   fi
fi
...
yml_auth_get()
{
   ...
   if [ -z "$username" ] || [ -z "$password" ]; then
      ...
      echo "  - $username:$password" >>/tmp/yaml_openkill_auth
   fi
}
...
PROXY_GROUPS=$(ruby -ryaml -rYAML -I "/usr/share/openkill" -E UTF-8 -e "
   begin
...
begin
   ...
   threads << Thread.new do
      begin
         ...
         if enable_sniffer
            ...
            if append_sniffer_config && (custom_sniffer = safe_load_yaml('/etc/openkill/custom/openkill_custom_sniffer.yaml'))
   ...
   threads << Thread.new do
      ...
      begin
         if '$custom_fallback_filter' == '1'
            ...
            elsif (filter_config = safe_load_yaml('/etc/openkill/custom/openkill_custom_fallback_filter.yaml'))
   ...
   threads << Thread.new do
      begin
         if custom_name_policy
            if (policy = safe_load_yaml('/etc/openkill/custom/openkill_custom_domain_dns_policy.list'))
   ...
   threads << Thread.new do
      begin
         if custom_proxy_server_policy
            if (policy = safe_load_yaml('/etc/openkill/custom/openkill_custom_proxy_server_dns_policy.list'))
      ...
      begin
         if custom_fakeip_filter
            ...
            if fake_ip_mode == 'fake-ip'
               merge_list_from_file(Value['dns'], 'fake-ip-filter', '/etc/openkill/custom/openkill_custom_fake_filter.list')
               merge_list_from_file(Value['dns'], 'fake-ip-filter', '/tmp/yaml_openkill_fake_filter_include')
            end
         end
   ...
   threads << Thread.new do
      begin
         if custom_host
            if (hosts_content = safe_load_yaml('/etc/openkill/custom/openkill_custom_hosts.list')) && !hosts_content.empty?
   ...
   threads << Thread.new do
      begin
         if (auth_config = safe_load_yaml('/tmp/yaml_openkill_auth'))
