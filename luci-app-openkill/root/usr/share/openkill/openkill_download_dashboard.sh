...
. /usr/share/openkill/uci.sh
   ...
   set_lock() {
      exec 871>"/tmp/lock/openkill_dashboard.lock" 2>/dev/null
      flock -x 871 2>/dev/null
   }

   del_lock() {
      flock -u 871 2>/dev/null
      rm -rf "/tmp/lock/openkill_dashboard.lock" 2>/dev/null
   }
...
EOF
   ...
   if [ "$DASH_NAME" == "Dashboard" ]; then
      UNPACK_FILE_DIR="/usr/share/openkill/ui/dashboard/"
      ...
      UNPACK_FILE_DIR="/usr/share/openkill/ui/yacd/"
		if [ "$DASH_TYPE" == "Official" ]; then
      ...
      UNPACK_FILE_DIR="/usr/share/openkill/ui/zashboard/"
      ...
      UNPACK_FILE_DIR="/usr/share/openkill/ui/metacubexd/"
   ...
   NEW_FILE_DIR="${TARGET_PARENT_DIR}/.openkill_dashboard_new.$$"
   OLD_FILE_DIR="${TARGET_PARENT_DIR}/.openkill_dashboard_old.$$"
