...
set_lock() {
   exec 883>"/tmp/lock/openkill_cus_domian.lock" 2>/dev/null
   flock -x 883 2>/dev/null
}

del_lock() {
   flock -u 883 2>/dev/null
   rm -rf "/tmp/lock/openkill_cus_domian.lock"
}
...
if [ "$(uci_get_config "enable_custom_domain_dns_server")" = "1" ] && [ "$(uci_get_config "enable_redirect_dns")" = "1" ]; then
   ...
   if [ -s "/etc/openkill/custom/openkill_custom_domain_dns.list" ]; then
      mkdir -p ${DNSMASQ_CONF_DIR}
      awk -v tag="$custom_domain_dns_server" '!/^$/&&!/^#/{printf("server=/%s/"'tag'"\n",$0)}' /etc/openkill/custom/openkill_custom_domain_dns.list >>${DNSMASQ_CONF_DIR}/dnsmasq_openkill_custom_domain.conf 2>/dev/null
   fi
fi
