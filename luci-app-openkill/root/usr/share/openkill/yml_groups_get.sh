...
set_lock() {
   exec 876>"/tmp/lock/openkill_groups_get.lock" 2>/dev/null
   flock -x 876 2>/dev/null
}

del_lock() {
   flock -u 876 2>/dev/null
   rm -rf "/tmp/lock/openkill_groups_get.lock"
}
...
ruby -ryaml -rYAML -I "/usr/share/openkill" -E UTF-8 -e "
   ...
   Value['proxy-groups'].each_with_index do |x, index|
      uci_name_tmp << %x{uci -q add openkill groups 2>&1}.chomp
      queue.push(nil)
      threadsp << Thread.new {
      begin
...
/usr/share/openkill/yml_proxys_get.sh "$CONFIG_FILE" >/dev/null 2>&1
del_lock