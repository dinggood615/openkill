...
openkill_openvpn_write_state()
{
   ...
   : > "$tmp" || return 1
   for key in requested generated applied role protocol ports endpoint4 endpoint6 client4 client6 reason updated; do
...
openkill_openvpn_prepare()
{
   ...
   case "$protocol" in tcp|udp) ;; *) protocol=invalid ;; esac
   ...
   if [ "$enabled" != 1 ] || [ "$transport" != 1 ]; then
      ...
      : > "$OPENKILL_OPENVPN_DIR/endpoints4.new"
      : > "$OPENKILL_OPENVPN_DIR/endpoints6.new"
      : > "$OPENKILL_OPENVPN_DIR/clients4.new"
      : > "$OPENKILL_OPENVPN_DIR/clients6.new"
      : > "$OPENKILL_OPENVPN_DIR/ports.new"
   ...
   mv -f "$v4_tmp" "$OPENKILL_OPENVPN_DIR/endpoints4.new"
   mv -f "$v6_tmp" "$OPENKILL_OPENVPN_DIR/endpoints6.new"
   mv -f "$clients4_tmp" "$OPENKILL_OPENVPN_DIR/clients4.new"
   mv -f "$clients6_tmp" "$OPENKILL_OPENVPN_DIR/clients6.new"
   mv -f "$ports_tmp" "$OPENKILL_OPENVPN_DIR/ports.new"
   openkill_openvpn_write_state || true
   return 0
}
...
openkill_openvpn_apply_runtime()
{
   ...
   if [ -n "$FW4" ] && command -v nft >/dev/null 2>&1; then
      ...
      command -v ipset >/dev/null 2>&1 || return 1
      batch="/tmp/openkill-openvpn-all.restore.$$"
      {
         for value in openkill_openvpn_endpoints4 openkill_openvpn_endpoints6 openkill_openvpn_clients4 openkill_openvpn_clients6 openkill_openvpn_ports; do
            printf 'flush %s\n' "$value"
         done
         while IFS= read -r value; do [ -n "$value" ] && printf 'add openkill_openvpn_endpoints4 %s\n' "$value"; done < "$source4"
         while IFS= read -r value; do [ -n "$value" ] && printf 'add openkill_openvpn_endpoints6 %s\n' "$value"; done < "$source6"
         while IFS= read -r value; do [ -n "$value" ] && printf 'add openkill_openvpn_clients4 %s\n' "$value"; done < "$clients4"
         while IFS= read -r value; do [ -n "$value" ] && printf 'add openkill_openvpn_clients6 %s\n' "$value"; done < "$clients6"
         while IFS= read -r value; do [ -n "$value" ] && printf 'add openkill_openvpn_ports %s\n' "$value"; done < "$ports"
      } > "$batch" || { rm -f "$batch"; return 1; }
      ipset restore < "$batch" >/dev/null 2>&1
      rc=$?
      rm -f "$batch"
...
{
   local chain family target clientset rule
   ...
   for chain in openkill openkill_mangle openkill_mangle_output openkill_output; do
      ...
      for family in 4 6; do
         ...
         if [ "$OPENKILL_OPENVPN_ROLE" = lan-client ]; then
            ...
            if [ "$family" = 4 ]; then
               rule="insert rule inet fw4 $chain position 0 meta l4proto { tcp, udp } ip saddr @${clientset} ip daddr @${target} th dport @openkill_openvpn_ports counter return comment \"OpenKill OpenVPN Scoped Transport\""
            else
               rule="insert rule inet fw4 $chain position 0 meta l4proto { tcp, udp } ip6 saddr @${clientset} ip6 daddr @${target} th dport @openkill_openvpn_ports counter return comment \"OpenKill OpenVPN Scoped Transport\""
            fi
         else
            if [ "$family" = 4 ]; then
               rule="insert rule inet fw4 $chain position 0 meta l4proto { tcp, udp } ip daddr @${target} th dport @openkill_openvpn_ports counter return comment \"OpenKill OpenVPN Scoped Transport\""
            else
               rule="insert rule inet fw4 $chain position 0 meta l4proto { tcp, udp } ip6 daddr @${target} th dport @openkill_openvpn_ports counter return comment \"OpenKill OpenVPN Scoped Transport\""
            fi
         fi
...
openkill_openvpn_add_legacy_rules()
{
   ...
   for table in nat mangle; do
      for chain in openkill openkill_output; do
         if [ "$OPENKILL_OPENVPN_ROLE" = lan-client ]; then
            rule="-p tcp -m set --match-set openkill_openvpn_clients4 src -m set --match-set openkill_openvpn_endpoints4 dst -m set --match-set openkill_openvpn_ports dst -m comment --comment OpenKill-OpenVPN-Scoped -j RETURN"
            iptables -t "$table" -nL "$chain" >/dev/null 2>&1 && iptables -t "$table" -I "$chain" 1 $rule 2>/dev/null || true
            rule="-p udp -m set --match-set openkill_openvpn_clients4 src -m set --match-set openkill_openvpn_endpoints4 dst -m set --match-set openkill_openvpn_ports dst -m comment --comment OpenKill-OpenVPN-Scoped -j RETURN"
            iptables -t "$table" -nL "$chain" >/dev/null 2>&1 && iptables -t "$table" -I "$chain" 1 $rule 2>/dev/null || true
            rule="-p tcp -m set --match-set openkill_openvpn_clients6 src -m set --match-set openkill_openvpn_endpoints6 dst -m set --match-set openkill_openvpn_ports dst -m comment --comment OpenKill-OpenVPN-Scoped -j RETURN"
            ip6tables -t "$table" -nL "$chain" >/dev/null 2>&1 && ip6tables -t "$table" -I "$chain" 1 $rule 2>/dev/null || true
            rule="-p udp -m set --match-set openkill_openvpn_clients6 src -m set --match-set openkill_openvpn_endpoints6 dst -m set --match-set openkill_openvpn_ports dst -m comment --comment OpenKill-OpenVPN-Scoped -j RETURN"
            ip6tables -t "$table" -nL "$chain" >/dev/null 2>&1 && ip6tables -t "$table" -I "$chain" 1 $rule 2>/dev/null || true
         else
            for proto in tcp udp; do
               rule="-p $proto -m set --match-set openkill_openvpn_endpoints4 dst -m set --match-set openkill_openvpn_ports dst -m comment --comment OpenKill-OpenVPN-Scoped -j RETURN"
               iptables -t "$table" -nL "$chain" >/dev/null 2>&1 && iptables -t "$table" -I "$chain" 1 $rule 2>/dev/null || true
               rule="-p $proto -m set --match-set openkill_openvpn_endpoints6 dst -m set --match-set openkill_openvpn_ports dst -m comment --comment OpenKill-OpenVPN-Scoped -j RETURN"
               ip6tables -t "$table" -nL "$chain" >/dev/null 2>&1 && ip6tables -t "$table" -I "$chain" 1 $rule 2>/dev/null || true
            done
         fi
      done
