...
set_lock() {
   exec 888>"/tmp/lock/openkill_update_databases.lock" 2>/dev/null
   flock -x 888 2>/dev/null
}

del_lock() {
   flock -u 888 2>/dev/null
   rm -rf "/tmp/lock/openkill_update_databases.lock" 2>/dev/null
}
   ...
   if [ "$DOWNLOAD_RESULT" -eq 0 ] && [ -s "/tmp/$tmpfile" ]; then
      if head -c 512 "/tmp/$tmpfile" | grep -qiE "<!doctype|<html|<head|<body"; then
         ...
         if [ "$?" -ne 0 ]; then
            ...
            case "$type" in
               geoip)
                  rm -rf "/etc/openkill/geoip.dat"
                  ;;
               geosite)
                  rm -rf "/etc/openkill/geosite.dat"
                  ;;
               geoasn)
                  rm -rf "/etc/openkill/GeoLite2-ASN.mmdb"
                  ;;
            esac
...
case "$1" in
   all)
      update_one geoip "geoip_custom_url" "GeoIP.dat" "/etc/openkill/GeoIP.dat" "GeoIP.dat"
      update_one geosite "geosite_custom_url" "GeoSite.dat" "/etc/openkill/GeoSite.dat" "GeoSite.dat"
      update_one geoasn "geoasn_custom_url" "GeoLite2-ASN.mmdb" "/etc/openkill/ASN.mmdb" "ASN.mmdb"
      update_one ipdb "geo_custom_url" "Country.mmdb" "/etc/openkill/Country.mmdb" "Country.mmdb"
      ;;
   geoip)
      update_one geoip "geoip_custom_url" "GeoIP.dat" "/etc/openkill/GeoIP.dat" "GeoIP.dat"
      ;;
