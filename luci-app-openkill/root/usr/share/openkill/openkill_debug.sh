...
del_lock() {
   ...
   rm -rf "/tmp/lock/openkill_debug.lock" 2>/dev/null
}
...
DEBUG_LOG="/tmp/openkill_debug.log"
...
\`\`\`bash
# df -h / /tmp /etc/openkill
$(df -h / /tmp /etc/openkill 2>/dev/null)
\`\`\`
...
if [ ! -f "/etc/openkill/core/clash_meta" ]; then
cat >> "$DEBUG_LOG" <<-EOF
| Meta 内核文件 | 不存在 |
EOF
else
...
if [ ! -x "/etc/openkill/core/clash_meta" ]; then
cat >> "$DEBUG_LOG" <<-EOF
| Meta 内核运行权限 | 否 |
EOF
else
...
\`\`\`bash
# ls -lh /etc/openkill/Country.mmdb /etc/openkill/GeoIP.dat /etc/openkill/GeoSite.dat /etc/openkill/ASN.mmdb
...
\`\`\`bash
# crontab -l | grep -i openkill
$(crontab -l 2>/dev/null | grep -i openkill || echo "无 OpenKill 相关 cron 任务")
\`\`\`

## 覆写模块设置

\`\`\`bash
# uci show openkill.@overwrite[0]
$(uci -q show openkill.@overwrite[0])
\`\`\`
...
\`\`\`bash
# cat /etc/openkill/custom/openkill_custom_rules.list
EOF
cat /etc/openkill/custom/openkill_custom_rules.list >> "$DEBUG_LOG"
echo "" >> "$DEBUG_LOG"
cat >> "$DEBUG_LOG" <<-EOF
\`\`\`
...
\`\`\`bash
# cat /etc/openkill/custom/openkill_custom_rules_2.list
EOF
cat /etc/openkill/custom/openkill_custom_rules_2.list >> "$DEBUG_LOG"
echo "" >> "$DEBUG_LOG"
cat >> "$DEBUG_LOG" <<-EOF
\`\`\`
EOF
...
\`\`\`bash
# cat /etc/openkill/custom/openkill_custom_overwrite.sh
EOF
cat /etc/openkill/custom/openkill_custom_overwrite.sh >> "$DEBUG_LOG" 2>/dev/null
echo "" >> "$DEBUG_LOG"
cat >> "$DEBUG_LOG" <<-EOF
\`\`\`
EOF
...
\`\`\`bash
# cat /etc/openkill/custom/openkill_custom_firewall_rules.sh
EOF
cat /etc/openkill/custom/openkill_custom_firewall_rules.sh >> "$DEBUG_LOG" 2>/dev/null
echo "" >> "$DEBUG_LOG"
cat >> "$DEBUG_LOG" <<-EOF
\`\`\`
EOF
...
EOF
   ...
   for nft in "openkill" "openkill_mangle" "openkill_mangle_output" "openkill_output" "openkill_post" "openkill_wan_input" "openkill_dns_hijack" "openkill_dns_redirect" "openkill_v6" "openkill_mangle_v6" "openkill_mangle_output_v6" "openkill_output_v6" "openkill_post_v6" "openkill_wan6_input"; do
      nft list chain inet fw4 "$nft" >> "$DEBUG_LOG" 2>/dev/null && echo "" >> "$DEBUG_LOG"
   done >/dev/null 2>&1
echo "" >> "$DEBUG_LOG"
cat >> "$DEBUG_LOG" <<-EOF
...
\`\`\`bash
# openkill_debug_dns.lua www.instagram.com
EOF
/usr/share/openkill/openkill_debug_dns.lua "www.instagram.com" >> "$DEBUG_LOG" 2>/dev/null
echo "" >> "$DEBUG_LOG"
cat >> "$DEBUG_LOG" <<-EOF
\`\`\`
EOF
...
tail -n 100 "/tmp/openkill.log" >> "$DEBUG_LOG" 2>/dev/null
echo "" >> "$DEBUG_LOG"
cat >> "$DEBUG_LOG" <<-EOF
\`\`\`
...
\`\`\`bash
# openkill_debug_getcon.lua
EOF
/usr/share/openkill/openkill_debug_getcon.lua
echo "" >> "$DEBUG_LOG"
cat >> "$DEBUG_LOG" <<-EOF
\`\`\`
EOF
