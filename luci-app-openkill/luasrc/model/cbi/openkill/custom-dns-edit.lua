...
m = Map(openkill, translate("Add Custom DNS Servers"))
m.pageaction = false
m.redirect = DISP.build_url("admin/services/openkill/config-overwrite")
if m.uci:get(openkill, sid) ~= "dns_servers" then
...
filename = m.uci:get(openkill, "config", "config_path")
if filename then
	groupnames = SYS.exec(string.format('ruby -ryaml -rYAML -I "/usr/share/openkill" -E UTF-8 -e "YAML.load_file(\'%s\')[\'proxy-groups\'].each do |i| puts i[\'name\']+\'##\' end" 2>/dev/null',filename))
	if groupnames then
