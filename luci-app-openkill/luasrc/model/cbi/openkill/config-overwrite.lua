...
local op_mode = string.sub(luci.sys.exec('uci get openkill.config.operation_mode 2>/dev/null'),0,-2)
...
local de_int = SYS.exec("ip route |grep 'default' |awk '{print $5}' 2>/dev/null") or SYS.exec("/usr/share/openkill/openkill_get_network.lua 'dhcp'")
...
o.description = translate("Proxy The GitHub Address In The Config And OpenKill With Proxy(CDN) To Prevent File Download Failed.").." "..translate("Format Reference:").." ".."<a href='javascript:void(0)' onclick='javascript:return winOpen(\"https://ghfast.top/\")'>https://ghfast.top/</a>"
o:value("0", translate("Disable"))
...
o.description = font_red..bold_on..translate("Set OpenKill Upstream DNS Resolve Server")..bold_off..font_off
o.default = 0
...
function custom_fallback_filter.cfgvalue(self, section)
	return fs.readfile("/etc/openkill/custom/openkill_custom_fallback_filter.yaml") or ""
end
function custom_fallback_filter.write(self, section, value)
	if value then
		value = value:gsub("\r\n?", "\n")
		local old_value = fs.readfile("/etc/openkill/custom/openkill_custom_fallback_filter.yaml")
		if value ~= old_value then
			fs.writefile("/etc/openkill/custom/openkill_custom_fallback_filter.yaml", value)
		end
	end
	return true
end
...
function custom_fake_black.cfgvalue(self, section)
	return fs.readfile("/etc/openkill/custom/openkill_custom_fake_filter.list") or ""
end
function custom_fake_black.write(self, section, value)
	if value then
		value = value:gsub("\r\n?", "\n")
		local old_value = fs.readfile("/etc/openkill/custom/openkill_custom_fake_filter.list")
		if value ~= old_value then
			fs.writefile("/etc/openkill/custom/openkill_custom_fake_filter.list", value)
		end
	end
	return true
end
...
function custom_domain_dns_policy.cfgvalue(self, section)
	return fs.readfile("/etc/openkill/custom/openkill_custom_domain_dns_policy.list") or ""
end
function custom_domain_dns_policy.write(self, section, value)
	if value then
		value = value:gsub("\r\n?", "\n")
		local old_value = fs.readfile("/etc/openkill/custom/openkill_custom_domain_dns_policy.list")
		if value ~= old_value then
			fs.writefile("/etc/openkill/custom/openkill_custom_domain_dns_policy.list", value)
		end
	end
	return true
end
...
function custom_proxy_server_dns_policy.cfgvalue(self, section)
	return fs.readfile("/etc/openkill/custom/openkill_custom_proxy_server_dns_policy.list") or ""
end
function custom_proxy_server_dns_policy.write(self, section, value)
	if value then
		value = value:gsub("\r\n?", "\n")
		local old_value = fs.readfile("/etc/openkill/custom/openkill_custom_proxy_server_dns_policy.list")
		if value ~= old_value then
			fs.writefile("/etc/openkill/custom/openkill_custom_proxy_server_dns_policy.list", value)
		end
	end
	return true
end
...
function custom_hosts.cfgvalue(self, section)
	return fs.readfile("/etc/openkill/custom/openkill_custom_hosts.list") or ""
end
function custom_hosts.write(self, section, value)
	if value then
		value = value:gsub("\r\n?", "\n")
		local old_value = fs.readfile("/etc/openkill/custom/openkill_custom_hosts.list")
		if value ~= old_value then
			fs.writefile("/etc/openkill/custom/openkill_custom_hosts.list", value)
		end
	end
	return true
end
...
function sniffer_custom.cfgvalue(self, section)
	return fs.readfile("/etc/openkill/custom/openkill_custom_sniffer.yaml") or ""
end
function sniffer_custom.write(self, section, value)
	if value then
		value = value:gsub("\r\n?", "\n")
		local old_value = fs.readfile("/etc/openkill/custom/openkill_custom_sniffer.yaml")
		if value ~= old_value then
			fs.writefile("/etc/openkill/custom/openkill_custom_sniffer.yaml", value)
		end
	end
	return true
end
...
o.description = translate("Current Version:").." "..font_green..bold_on..translate(fs.get_resourse_mtime("/etc/openkill/Model.bin"))..bold_off..font_off
...
o.write = function()
	...
	SYS.call("/usr/share/openkill/openkill_lgbm.sh >/dev/null 2>&1 &")
...
o.template = "openkill/flush_smart_cache"
...
function custom_rules.cfgvalue(self, section)
	return fs.readfile("/etc/openkill/custom/openkill_custom_rules.list") or ""
end
function custom_rules.write(self, section, value)
	if value then
		value = value:gsub("\r\n?", "\n")
		local old_value = fs.readfile("/etc/openkill/custom/openkill_custom_rules.list")
		if value ~= old_value then
			fs.writefile("/etc/openkill/custom/openkill_custom_rules.list", value)
		end
	end
	return true
end
...
function custom_rules_2.cfgvalue(self, section)
	return fs.readfile("/etc/openkill/custom/openkill_custom_rules_2.list") or ""
end
function custom_rules_2.write(self, section, value)
	if value then
		value = value:gsub("\r\n?", "\n")
		local old_value = fs.readfile("/etc/openkill/custom/openkill_custom_rules_2.list")
		if value ~= old_value then
			fs.writefile("/etc/openkill/custom/openkill_custom_rules_2.list", value)
		end
	end
	return true
end
...
ds.addremove = true
ds.sortable = true
ds.template = "openkill/tblsection"
ds.extedit = DISP.build_url("admin/services/openkill/custom-dns-edit/%s")
function ds.create(self, section)
	local sid = TypedSection.create(self, section)
	if sid then
		local name = HTTP.formvalue("cbi.cts.tagname.".. self.config .. "." .. self.sectiontype)
		if name and #name > 0 then
			self.map.uci:set("openkill", sid, "group", name)
		end
