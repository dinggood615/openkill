...
function default_config_set(f)
	...
	if cf == "/etc/openkill/config/"..f or not cf or cf == "" or not fs.isfile(cf) then
		if CHIF == "1" and cf == "/etc/openkill/config/"..f then
			return
		end
		local fis = fs.glob("/etc/openkill/config/*")[1]
		if fis ~= nil then
			fcf = fs.basename(fis)
			if fcf then
				uci:set("openkill", "config", "config_path", "/etc/openkill/config/"..fcf)
				uci:commit("openkill")
			end
		else
			uci:set("openkill", "config", "config_path", "/etc/openkill/config/config.yaml")
			uci:commit("openkill")
		end
	end
end
...
o.template = "openkill/upload"
o.cfgvalue = function(self, section)
	return self.value
end

local dir, fd, clash
dir = "/etc/openkill/config/"
proxy_pro_dir="/etc/openkill/proxy_provider/"
rule_pro_dir="/etc/openkill/rule_provider/"
core_dir="/etc/openkill/core/core/"
backup_dir="/tmp/"
			...
			if fp == "config" then
				...
				if fs.IsYamlExt(meta.file) then
					if string.lower(meta.file):sub(-4) ~= ".yml" then
						...
						local c=fs.rename(dir .. meta.file,"/etc/openkill/config/".. ymlname .. ".yaml")
						default_config_set(ymlname .. ".yaml")
					end
				end
				o.value = translate("File saved to") .. ' "/etc/openkill/config/"'
			elseif fp == "proxy-provider" then
				o.value = translate("File saved to") .. ' "/etc/openkill/proxy_provider/"'
			elseif fp == "rule-provider" then
				o.value = translate("File saved to") .. ' "/etc/openkill/rule_provider/"'
			elseif fp == "clash_meta" then
				local archive_path = core_dir .. meta.file
				if string.lower(string.sub(meta.file, -7, -1)) == ".tar.gz" then
					-- tar.gz
					os.execute(string.format("tar -C '/etc/openkill/core/core' -xzf '%s' >/dev/null 2>&1", archive_path))
					os.execute(string.format("rm -f '%s' >/dev/null 2>&1", archive_path))
					local first_file_cmd = "find /etc/openkill/core/core -type f ! -name '*.tar.gz' ! -name '*.tar' ! -name '*.gz' 2>/dev/null | head -1"
					local first_file = io.popen(first_file_cmd):read("*line")
					if first_file and first_file ~= "" then
						os.execute(string.format("mv '%s' '/etc/openkill/core/%s' >/dev/null 2>&1", first_file, fp))
					end
				elseif string.lower(string.sub(meta.file, -4, -1)) == ".tar" then
					-- tar
					os.execute(string.format("tar -C '/etc/openkill/core/core' -xf '%s' >/dev/null 2>&1", archive_path))
					os.execute(string.format("rm -f '%s' >/dev/null 2>&1", archive_path))
					local first_file_cmd = "find /etc/openkill/core/core -type f ! -name '*.tar' ! -name '*.gz' 2>/dev/null | head -1"
					local first_file = io.popen(first_file_cmd):read("*line")
					if first_file and first_file ~= "" then
						os.execute(string.format("mv '%s' '/etc/openkill/core/%s' >/dev/null 2>&1", first_file, fp))
					end
				elseif string.lower(string.sub(meta.file, -3, -1)) == ".gz" then
					...
					os.execute(string.format("rm -f '%s' >/dev/null 2>&1", archive_path))
					local first_file_cmd = "find /etc/openkill/core/core -type f ! -name '*.gz' 2>/dev/null | head -1"
					local first_file = io.popen(first_file_cmd):read("*line")
					if first_file and first_file ~= "" then
						os.execute(string.format("mv '%s' '/etc/openkill/core/%s' >/dev/null 2>&1", first_file, fp))
					end
				else
					os.execute(string.format("mv '%s' '/etc/openkill/core/%s' >/dev/null 2>&1", (core_dir .. meta.file), fp))
				end
				
				os.execute(string.format("chmod 4755 '/etc/openkill/core/%s' >/dev/null 2>&1", fp))
				os.execute(string.format("rm -rf %s >/dev/null 2>&1", core_dir))
				o.value = translate("File saved to") .. ' "/etc/openkill/core/"'
			elseif fp == "backup-file" then
				os.execute("tar -C '/etc/openkill/' -xzf %s >/dev/null 2>&1" % (backup_dir .. meta.file))
				os.execute("mv /etc/openkill/openkill /etc/config/openkill >/dev/null 2>&1")
...
st.template="openkill/cfg_check"
sb.template="openkill/sub_info_show"
...
btnis.write=function(a,t)
	uci:set("openkill", "config", "config_path", "/etc/openkill/config/"..e[t].name)
	uci:commit("openkill")
	HTTP.redirect(DISP.build_url("admin", "services", "openkill", "config"))
end

btned=tb:option(Button,"edit",translate("Edit"))
btned.inputstyle="apply"
btned.write=function(a,t)
	local file_path = "/etc/openkill/config/" .. fs.basename(e[t].name)
	HTTP.redirect(DISP.build_url("admin", "services", "openkill", "other-file-edit", "config") .. "?file=" .. HTTP.urlencode(file_path))
end

btnrn=tb:option(DummyValue,"/etc/openkill/config/",translate("Rename"))
btnrn.template="openkill/input_rename"
btnrn.rawhtml = true
...
actions.render = function(self, t, a)
	...
	-- Download Running
	if NXFS.access("/etc/openkill/"..e[t].name) then
...
btnapply.write = function(self, t)
	...
	if action == "servers_manage" then
		local file_path = "/etc/openkill/config/" .. fs.basename(e[t].name)
		HTTP.redirect(DISP.build_url("admin", "services", "openkill", "servers") .. "?file=" .. HTTP.urlencode(file_path))
	elseif action == "copy" then
		local num = 1
		while true do
			num = num + 1
			if not fs.isfile("/etc/openkill/config/"..fs.filename(e[t].name).."("..num..")"..".yaml") then
				fs.copy("/etc/openkill/config/"..e[t].name, "/etc/openkill/config/"..fs.filename(e[t].name).."("..num..")"..".yaml")
				break
			end
		end
		HTTP.redirect(DISP.build_url("admin", "services", "openkill", "config"))
	elseif action == "download" then
		...
		sPath = "/etc/openkill/config/"..e[t].name
	...
	elseif action == "remove" then
		...
		fs.unlink("/etc/openkill/history/"..fs.filename(e[t].name)..".db")
		fs.unlink("/etc/openkill/"..file_name)
		local a=fs.unlink("/etc/openkill/config/"..file_name)
...
o.inputstyle = "reload"
o.write = function()
	HTTP.redirect(DISP.build_url("admin", "services", "openkill", "proxy-provider-file-manage"))
end
...
o.inputstyle = "reload"
o.write = function()
	HTTP.redirect(DISP.build_url("admin", "services", "openkill", "rule-providers-file-manage"))
end

m = SimpleForm("openkill",translate("Config File Edit"))
m.reset = false
m.submit = false
...
local dconf = "/usr/share/openkill/res/default.yaml"
if not conf then conf = "/etc/openkill/config/config.yaml" end
local conf_name = fs.basename(conf)
if not conf_name then conf_name = "config.yaml"  end
local sconf = "/etc/openkill/"..conf_name
...
if fs.isfile(sconf) then
	---def.description = align_mid..translate("Config File Edited By OpenKill For Running")..align_mid_off
else
...
o.rawhtml = true
o.template = "openkill/input_file_name"
o.value = "/etc/openkill/config/"
...
m:append(Template("openkill/config_merge_editor"))
