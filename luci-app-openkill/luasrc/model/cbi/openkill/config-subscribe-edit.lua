...
m = Map(openkill, translate("Config Subscribe Edit"))
m.pageaction = false
...
"<br/>"..
"<br/>"..translate("If you have a recommended external configuration file (subscription conversion template), you can modify by following The file format of /usr/share/openkill/res/sub_ini.list and pr")
m.redirect = DISP.build_url("admin/services/openkill/config-subscribe")
if m.uci:get(openkill, sid) ~= "config_subscribe" then
...
function o.cfgvalue(self, section)
	local name = m.uci:get(openkill, section, "name") or section
	local v = ""
	m.uci:foreach(openkill, "config_age_secret", function(s)
		if s.name == name then
			v = s.algo or ""
			age_section = s['.name']
			return false
...
function o.write(self, section, value)
	...
	if not age_section and value and value ~= "" then
		age_section = m.uci:add(openkill, "config_age_secret")
		if age_section then m.uci:set(openkill, age_section, "name", name) end
	end
	if age_section then
		if value and value ~= "" then
			m.uci:set(openkill, age_section, "algo", value)
		else
			m.uci:delete(openkill, age_section, "algo")
		end
	end
end
...
function o.write(self, section, value)
	...
	if age_section then
		if value and value ~= "" then
			m.uci:set(openkill, age_section, "secret", value)
		else
			m.uci:delete(openkill, age_section, "secret")
		end
	end
end
function o.remove(self, section)
...
function o.write(self, section, value)
	...
	if age_section then
		if value and value ~= "" then
			m.uci:set(openkill, age_section, "public", value)
		else
			m.uci:delete(openkill, age_section, "public")
		end
	end
end
function o.remove(self, section)
...
o.template = "openkill/generate_age"
...
o.rmempty = true
o:depends("sub_convert", "1")
file = io.open("/usr/share/openkill/res/sub_ini.list", "r");
...
o.write = function()
	local to_delete = {}
	m.uci:foreach(openkill, "config_age_secret", function(s)
		local pub = m.uci:get(openkill, s['.name'], "public") or ""
		local sec = m.uci:get(openkill, s['.name'], "secret") or ""
		if (pub == "" or pub == nil) and (sec == "" or sec == nil) then
			table.insert(to_delete, s['.name'])
		end
	end)
	for _, n in ipairs(to_delete) do
		m.uci:delete(openkill, n)
	end
...
o.write = function()
	...
	m.uci:revert(openkill, age_section)
	HTTP.redirect(m.redirect)
end

m:append(Template("openkill/toolbar_show"))
m:append(Template("openkill/subconverter_version"))
return m
