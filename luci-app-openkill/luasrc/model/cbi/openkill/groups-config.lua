...
if not file_path then
	HTTP.redirect(DISP.build_url("admin", "services", "openkill", "servers"))
	return
end
...
m = Map(openkill, translate("Edit Group"))
m.pageaction = false
m.redirect = DISP.build_url("admin/services/openkill/servers") .. "?file=" .. HTTP.urlencode(file_path)
if m.uci:get(openkill, sid) ~= "groups" then
...
local e,a={}
for t,f in ipairs(fs.glob("/etc/openkill/config/*"))do
	a=fs.stat(f)
	if a then
		e[t]={}
...
uci:foreach("openkill", "groups",
	function(s)
		if s.name ~= "" and s.name ~= nil and s.name ~= m.uci:get(openkill, sid, "name") and (s.config == m.uci:get(openkill, sid, "config") or s.config == "all") then
			o:value(s.name)
		end
	end)
o:value("DIRECT")
...
local function sync_group_name(section, old_name, new_name)
	...
	-- servers 的 groups 列表
	uci:foreach(openkill, "servers", function(s)
		local groups = uci:get(openkill, s[".name"], "groups")
		if groups then
			local new_groups = {}
			local changed = false
			...
			if changed then
				uci:delete(openkill, s[".name"], "groups")
				uci:set_list(openkill, s[".name"], "groups", new_groups)
			end
		end
	end)

	-- proxy-provider 的 groups 列表
	uci:foreach(openkill, "proxy-provider", function(s)
		local groups = uci:get(openkill, s[".name"], "groups")
		if groups then
			local new_groups = {}
			local changed = false
	...
	uci:foreach(openkill, "groups", function(s)
		if s[".name"] ~= section then
			local other_group = uci:get(openkill, s[".name"], "other_group")
			if other_group then
				local new_other = {}
				local changed = false
				...
				if changed then
					uci:delete(openkill, s[".name"], "other_group")
					uci:set_list(openkill, s[".name"], "other_group", new_other)
				end
			end
		end
	end)
	...
	uci:foreach(openkill, "dns_servers", function(s)
		local specific_group = uci:get(openkill, s[".name"], "specific_group")
		if matches(old_name, specific_group) then
			uci:set(openkill, s[".name"], "specific_group", new_name)
		end
	end)

	-- servers 的 dialer_proxy 选项
	uci:foreach(openkill, "servers", function(s)
		local dialer_proxy = uci:get(openkill, s[".name"], "dialer_proxy")
		if matches(old_name, dialer_proxy) then
			uci:set(openkill, s[".name"], "dialer_proxy", new_name)
		end
	end)
...
o.inputstyle = "apply"
o.write = function()
	local old_name = m.uci:get(openkill, sid, "old_name") or ""
	local new_name = HTTP.formvalue("cbid.openkill." .. sid .. ".name") or m.uci:get(openkill, sid, "name")
	sync_group_name(sid, old_name, new_name)
	m.uci:set(openkill, sid, "old_name", new_name)
