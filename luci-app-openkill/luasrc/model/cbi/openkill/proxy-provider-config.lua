...
m = Map(openkill, translate("Edit Proxy-Provider"))
m.pageaction = false
m.redirect = DISP.build_url("admin/services/openkill/servers") .. "?file=" .. HTTP.urlencode(file_path)
if m.uci:get(openkill, sid) ~= "proxy-provider" then
...
local p,h={}
for t,f in ipairs(fs.glob("/etc/openkill/proxy_provider/*"))do
	h=fs.stat(f)
	if h then
		p[t]={}
...
m.uci:foreach("openkill", "groups",
		function(s)
			if s.name ~= "" and s.name ~= nil and (s.config == m.uci:get(openkill, sid, "config") or s.config == "all") then
				o:value(s.name)
			end
		end)
