...
if not file_path then
	HTTP.redirect(DISP.build_url("admin", "services", "openkill", "%s") % arg[1])
	return
end

m = Map("openkill", translate("File Edit"))
m.pageaction = false
m.redirect = DISP.build_url("admin", "services", "openkill", "other-file-edit", "%s") % arg[1].."?file="..HTTP.urlencode(file_path)
