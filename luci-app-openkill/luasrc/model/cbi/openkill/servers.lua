...
m = Map(openkill, translate("Servers & Groups manage"))
m.pageaction = false
...
o.inputstyle = "reset"
o.write = function()
	m.uci:foreach("openkill", "servers",
	function(s)
		if s.enabled ~= "1" then
			m.uci:delete("openkill", s[".name"])
		end
	end)
...
o.inputstyle = "reset"
o.write = function()
	m.uci:delete_all("openkill", "servers", function(s) return true end)
	m.uci:commit("openkill")
	HTTP.redirect(m.redirect)
end
...
o.inputstyle = "reset"
o.write = function()
	m.uci:delete_all("openkill", "proxy-provider", function(s) return true end)
	m.uci:commit("openkill")
	HTTP.redirect(m.redirect)
end
...
o.inputstyle = "reset"
o.write = function()
	m.uci:delete_all("openkill", "groups", function(s) return true end)
	m.uci:commit("openkill")
	HTTP.redirect(m.redirect)
end
...
o.write = function()
	m.uci:commit("openkill")
	luci.sys.call("/usr/share/openkill/yml_groups_get.sh \"%s\" 2>/dev/null" % file_path)
	HTTP.redirect(m.redirect)
end
...
o.write = function()
	m.uci:commit("openkill")
	luci.sys.call("/usr/share/openkill/yml_groups_set.sh \"%s\" >/dev/null 2>&1 &" % file_path)
	HTTP.redirect(m.redirect)
end
