...
local p,r={}
for x,y in ipairs(fs.glob("/etc/openkill/proxy_provider/*"))do
r=fs.stat(y)
if r then
p[x]={}
...
btned1.write=function(r,x)
	local file_path = "/etc/openkill/proxy_provider/" .. fs.basename(p[x].name)
	HTTP.redirect(DISP.build_url("admin", "services", "openkill", "other-file-edit", "proxy-provider-file-manage") .. "?file=" .. HTTP.urlencode(file_path))
end
...
btndl1.write = function (r,x)
	...
	sPath = "/etc/openkill/proxy_provider/"..p[x].name
...
local r=fs.unlink("/etc/openkill/proxy_provider/"..luci.openkill.basename(p[x].name))
...
o.value = "/etc/openkill/proxy_provider/"

o = a:option(Button, "Delete_all", " ")
o.inputtitle = translate("Delete All File")
o.inputstyle = "remove"
o.write = function()
	luci.sys.call("rm -rf /etc/openkill/proxy_provider/* >/dev/null 2>&1")
	HTTP.redirect(DISP.build_url("admin", "services", "openkill", "proxy-provider-file-manage"))
end
...
proxy_form:append(Template("openkill/toolbar_show"))
return proxy_form
