...
local g,h={}
for n,m in ipairs(fs.glob("/etc/openkill/rule_provider/*"))do
h=fs.stat(m)
if h then
g[n]={}
...
btned1.write=function(h,n)
	local file_path = "/etc/openkill/rule_provider/" .. fs.basename(g[n].name)
	HTTP.redirect(DISP.build_url("admin", "services", "openkill", "other-file-edit", "rule-providers-file-manage") .. "?file=" .. HTTP.urlencode(file_path))
end
...
btndl2.write = function (h,n)
	...
	sPath = "/etc/openkill/rule_provider/"..g[n].name
...
local h=fs.unlink("/etc/openkill/rule_provider/"..luci.openkill.basename(g[n].name))
...
o.value = "/etc/openkill/rule_provider/"

o = a:option(Button, "Delete_all", " ")
o.inputtitle = translate("Delete All File")
o.inputstyle = "remove"
o.write = function()
	luci.sys.call("rm -rf /etc/openkill/rule_provider/* >/dev/null 2>&1")
	HTTP.redirect(DISP.build_url("admin", "services", "openkill", "rule-providers-file-manage"))
end
...
rule_form:append(Template("openkill/toolbar_show"))
return rule_form
