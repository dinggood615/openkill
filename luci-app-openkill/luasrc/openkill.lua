...
function lanip()
	local lan_int_name = uci:get("openkill", "@overwrite[0]", "lan_interface_name") or uci:get("openkill", "config", "lan_interface_name") or "0"
	local lan_ip
...
function get_resourse_mtime(path)
    ...
    if not fs.access(path) then
        ...
        elseif uci_get_config("config", "small_flash_memory") == "1" then
            local fallback_path = path:gsub("^/etc/openkill/", "/tmp/etc/openkill/")
	...
	local resourse_etag_version = SYS.exec(string.format("source /usr/share/openkill/openkill_etag.sh && GET_ETAG_TIMESTAMP_BY_PATH '%s'", file))
...
function uci_get_config(section, key)
	...
	if section == "config" then
    	val = uci:get("openkill", "@overwrite[0]", key)
	end
    if val == nil then
    	val = uci:get("openkill", section, key)
    end
    return val
end
...
function age_encrypt(public, content)
    ...
    local cmd = string.format(
        "/etc/openkill/core/clash_meta age encrypt %s %s %s 2>/dev/null",
...
function age_decrypt(secret, content)
    ...
    local cmd = string.format(
        "/etc/openkill/core/clash_meta age decrypt %s %s %s 2>/dev/null",
...
function config_refs(old_file_name, new_file_name)
		...
		function(s)
			if s.name == old_name_no_ext then
				if is_rename and new_name_no_ext ~= new_file_name then
					uci:set("openkill", s[".name"], "name", new_name_no_ext)
				else
	...
	uci:foreach("openkill", "groups",
		function(s)
			if s.config == old_file_full then
				if is_rename and new_name_no_ext ~= new_file_name then
					uci:set("openkill", s[".name"], "config", new_file_full)
				else
					uci:delete("openkill", s[".name"])
				end
			end
	...
	uci:foreach("openkill", "proxy-provider",
		function(s)
			if s.config == old_file_full then
				if is_rename and new_name_no_ext ~= new_file_name then
					uci:set("openkill", s[".name"], "config", new_file_full)
	...
	uci:foreach("openkill", "config_age_secret",
		function(s)
			if s.name == old_name_no_ext then
				if is_rename and new_name_no_ext ~= new_file_name then
					uci:set("openkill", s[".name"], "name", new_name_no_ext)
...
--- Reads the CDN proxy address list from /usr/share/openkill/res/cdn.list.
-- Lines starting with "#" are treated as comments and skipped.
-- @return Table of CDN address strings (deduplicated, in file order)
function cdn_list()
	local list = {}
	local seen = {}
	local raw = fs.readfile("/usr/share/openkill/res/cdn.list")
	if raw then
...
-- @param pkg   Package name, e.g. "luci-app-openkill" or "libc"
...
function oc_version()
	local v = read_pkg_field("luci-app-openkill", pkg_type() == "opkg" and "Version" or "V")
	if v == "" then
		v = "0"
	end
	return v
