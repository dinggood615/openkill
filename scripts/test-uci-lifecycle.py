...
class UciLifecycleContractTests(unittest.TestCase):
    ...
    def test_change_dnsmasq_declares_exact_runtime_delta(self):
        ...
        for marker in (
            ...
            'uci -q set "$DNSMASQ_UCI.localuse=1"',
            'uci -q set "$DNSMASQ_UCI.cachesize=0"',
            'uci -q set "$DNSMASQ_UCI.filter_aaaa=0"',
    ...
    def test_revert_dnsmasq_restores_or_converges_every_runtime_field(self):
        ...
        for marker in (
            ...
            'uci -q set "$DNSMASQ_UCI.noresolv=0"',
            'uci -q set "$DNSMASQ_UCI.localuse=1"',
            'uci -q set "$DNSMASQ_UCI.cachesize=$dnsmasq_cachesize"',
            'uci -q set "$DNSMASQ_UCI.filter_aaaa=$dnsmasq_filter_aaaa"',
