...
class DualStackRoutingTests(unittest.TestCase):
    ...
    def test_fw4_include_defers_openkill_rule_rebuild_after_interface_change(self):
        ...
        self.assertIn('SETTLE_DELAY="${OPENKILL_FW4_SETTLE_DELAY:-3}"', helper)
        self.assertIn('"$OPENKILL_INIT_SERVICE" reload "firewall-deferred"', helper)
        self.assertIn('OPENKILL_START_TOKEN_FILE="${OPENKILL_START_TOKEN_FILE:-/tmp/openkill-start.token}"', helper)
        self.assertIn('OPENKILL_READY_TOKEN_FILE="${OPENKILL_READY_TOKEN_FILE:-/tmp/openkill-ready.token}"', helper)
        self.assertIn('PENDING_FILE="$NETWORK_STATE_DIR/pending"', helper)
        self.assertIn('if (eval \'exec 1000>&-\')', helper)
        ...
        self.assertIn('"$OPENKILL_INIT_SERVICE" start', helper)
