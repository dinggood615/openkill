...
项目版本使用 `2026-1071` 形式；后续小改动递增 1，大改动跳到下一个整十（例如 `2026-1071` → `2026-1080`），APK 内部版本转换为 `2026.1071`，以满足 APK 包管理器的版本语法，数字序号保持同步。
...
2026-1071：将 TUN 路由与透明代理改为互斥的单一接管模式；默认由 OpenKill 管理路由、防火墙和 DNS，另提供 Mihomo 原生自动接管选项。修复 fw4 include 启动竞态、缺失 include 路径、OpenKill 路由等待死循环，并为两种模式补充语义校验和中文设置项。
