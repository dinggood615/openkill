...
项目版本使用 `2026-1061` 形式；后续小改动递增 1，大改动跳到下一个整十（例如 `2026-1061` → `2026-1070`），APK 内部版本转换为 `2026.1061`，以满足 APK 包管理器的版本语法，数字序号保持同步。
...
本次一次性整合（`2026-1060`）将四阶段收敛为可回滚的默认路径：生成配置先做语义检查再替换，控制器默认绑定 LAN 地址、DNS 默认仅监听回环地址，TUN 的路由权交由 OpenKill 防火墙统一管理；watchdog 只观察 procd/核心状态，低频执行规则、历史、UPnP 和节点地址维护，并为 DNS 劫持修复加入冷却窗口。节点测速统一采用有界超时、204 探测和懒加载，避免单个失联节点拖慢整个面板。旧 Smart/LightGBM/OixCloud 参数在安装和更新时清理，官方 Mihomo 内核仍按设备架构独立校验下载。
